...
func run(sourceConfigPath string, cursorPath string) error {
	...
	configPath := filepath.Join(dirs.home, ".code-work", "config.yaml")
	if err := writeConfig(configPath, isolatedConfig); err != nil {
		return err
	}

	certPath := filepath.Join(dirs.home, ".code-work", "data", "ca.crt")
	keyPath := filepath.Join(dirs.home, ".code-work", "data", "ca.key")
	...
	store := serverconfig.NewStore(configPath, filepath.Join(dirs.home, ".code-work", "logs"))
