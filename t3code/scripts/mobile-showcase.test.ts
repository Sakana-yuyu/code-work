...
it("selects an explicit CI Android ABI without changing the local default", () => {
  ...
  assert.throws(() => resolveShowcaseAndroidAbi("mips"), /Unsupported CODEWORK_SHOWCASE_ANDROID_ABI/u);
});
...
it("expands both appearances into independent upload-ready directories", () => {
  ...
  assert.deepStrictEqual(
    ...
    [
      { appearance: "light", directory: "/captures/apple/iphone-test/light/code-work" },
      { appearance: "dark", directory: "/captures/apple/iphone-test/dark/code-work" },
    ],
  );
});
...
it("defaults every device to the app's own palette", () => {
  assert.equal(DEFAULT_SHOWCASE_THEME, "code-work");
  assert.equal(
