...
const devServerInput = {
  mode: "dev:server",
  codeworkHome: "/tmp/codework-dev-runner",
  browser: undefined,
...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit CODEWORK_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "CODEWORK_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "CODEWORK_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("leaves the shared home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CODEWORK_HOME, undefined);
        assert.equal(env.CODEWORK_NO_BROWSER, "1");
      }),
    );
    ...
    it.effect("allows browser auto-open to be explicitly enabled", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CODEWORK_NO_BROWSER, "0");
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CODEWORK_HOME, path.resolve("/tmp/custom-t3"));
        assert.equal(env.CODEWORK_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.CODEWORK_NO_BROWSER, "1");
        assert.equal(env.CODEWORK_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.CODEWORK_LOG_WS_EVENTS, "1");
        assert.equal(env.CODEWORK_HOST, "0.0.0.0");
    ...
    it.effect("strips inherited service-launcher context", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            CODEWORK_SERVICE_LAUNCHER_CONTEXT: '{"childVersion":"9.9.9"}',
            CODEWORK_BOOT_SERVICE_UNIT: "codework.service",
          },
          serverOffset: 0,
          webOffset: 0,
          codeworkHome: undefined,
        ...
        assert.equal(env.CODEWORK_SERVICE_LAUNCHER_CONTEXT, undefined);
        assert.equal(env.CODEWORK_BOOT_SERVICE_UNIT, undefined);
      }),
    );
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            CODEWORK_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          codeworkHome: undefined,
        ...
        assert.equal(env.CODEWORK_MODE, "web");
        assert.equal(env.CODEWORK_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CODEWORK_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom codeworkHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.CODEWORK_HOME, path.resolve("/tmp/my-t3"));
      }),
    );
      ...
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            CODEWORK_PORT: "13773",
            CODEWORK_MODE: "web",
            CODEWORK_NO_BROWSER: "0",
            CODEWORK_HOST: "0.0.0.0",
        ...
        assert.equal(env.CODEWORK_MODE, undefined);
        assert.equal(env.CODEWORK_NO_BROWSER, undefined);
        assert.equal(env.CODEWORK_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CODEWORK_PORT, "13773");
        assert.equal(env.PORT, "5733");
      }),
    );
    ...
    for (const mode of ["dev", "dev:web"] as const) {
      it.effect(`leaves the client backend URLs unset in ${mode} mode`, () =>
        Effect.gen(function* () {
          ...
          assert.equal(env.CODEWORK_SINGLE_ORIGIN_DEV, "1");
        }),
      );
    }
    ...
    it.effect("clears the single-origin marker in dev:desktop mode", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: { CODEWORK_SINGLE_ORIGIN_DEV: "1" },
          serverOffset: 0,
          webOffset: 0,
          codeworkHome: undefined,
          browser: undefined,
        ...
        assert.equal(env.CODEWORK_SINGLE_ORIGIN_DEV, undefined);
    ...
    // --host configures the *backend* (CODEWORK_HOST). It must not become Vite's
  ...
  describe("devPortProbeHosts", () => {
    ...
    // actually bind, so --host/CODEWORK_HOST has to be probed as well.
    ...
    // Only the backend honours --host/CODEWORK_HOST. Vite reads HOST (set for
  ...
  describe("runDevRunnerWithInput", () => {
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { CODEWORK_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, ["CODEWORK_PORT_OFFSET", "CODEWORK_DEV_INSTANCE"]);
    ...
    // Base-dir precedence (--home-dir > worktree .t3 > ambient CODEWORK_HOME)
    ...
    it.effect("ignores an ambient CODEWORK_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { CODEWORK_HOME: "/home/user/.t3" },
          serverOffset: 0,
          webOffset: 0,
          codeworkHome: undefined,
          browser: undefined,
    ...
    // defaults: an explicit CODEWORK_BUNDLED_DEV (even "0") must pass through.
    describe("--share bundled dev default", () => {
          ...
          yield* runDevRunnerWithInput({
            ...
            Effect.provideService(
              ...
              input.ambientBundledDev === undefined
                ? {}
                : { CODEWORK_BUNDLED_DEV: input.ambientBundledDev },
            ),
          );
      ...
      it.effect("defaults CODEWORK_BUNDLED_DEV=1 for a shared run", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: undefined });
          assert.equal(env?.CODEWORK_BUNDLED_DEV, "1");
        }),
      );

      it.effect("keeps an explicit CODEWORK_BUNDLED_DEV=0 opt-out", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: "0" });
          assert.equal(env?.CODEWORK_BUNDLED_DEV, "0");
        }),
      );

      it.effect("leaves CODEWORK_BUNDLED_DEV unset without --share", () =>
        Effect.gen(function* () {
          let captured: Record<string, string | undefined> | undefined;
          const spawnerLayer = Layer.succeed(
            ChildProcessSpawner.ChildProcessSpawner,
          ...
          assert.equal(captured?.CODEWORK_BUNDLED_DEV, undefined);
        }),
      );
    });
    ...
    describe("t3 home precedence", () => {
      ...
      const spawnedHome = (input: {
        readonly codeworkHome: string | undefined;
        readonly cwd: string;
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({ ...devServerInput, codeworkHome: input.codeworkHome }).pipe(
            ...
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { CODEWORK_HOME: input.ambientHome },
            ),
          );

          return captured?.CODEWORK_HOME;
        });
      ...
      it.effect("prefers an explicit --home-dir over the worktree default", () =>
        Effect.gen(function* () {
          ...
          const home = yield* spawnedHome({
            codeworkHome: "/tmp/explicit-home",
            cwd: root,
      ...
      it.effect("prefers the worktree .t3 over an ambient CODEWORK_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient CODEWORK_HOME outside a worktree", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const home = yield* spawnedHome({
            codeworkHome: undefined,
