...
describe("buildProjectGroups", () => {
  it("preserves every physical clone as a selectable member in repository modes", () => {
    const projects = [
      makeProject("codework", "/work/codework"),
      makeProject("codework-2", "/work/codework-2"),
      makeProject("codework-3", "/work/codework-3"),
    ];
  ...
  it("uses a shared custom title as the repository group's label", () => {
    const projects = [
      makeProject("first", "/work/codework", { title: "Custom project" }),
      makeProject("second", "/work/codework-2", { title: "Custom project" }),
    ];
  ...
  it("keeps the repository label when shared titles match its repository name", () => {
    const projects = [
      makeProject("first", "/work/codework", { title: "codework" }),
      makeProject("second", "/work/codework-2", { title: "codework" }),
    ];
  ...
  it("keeps physical clones in separate groups when requested", () => {
    ...
    expect(groups.map((group) => group.label)).toEqual(["codework", "codework-2", "codework-3"]);
  });

  it("applies a physical-project override without dropping its siblings", () => {
    const first = makeProject("codework", "/work/codework");
    const second = makeProject("codework-2", "/work/codework-2");
    const third = makeProject("codework-3", "/work/codework-3");
  ...
  it("dedupes stale registrations at one physical path using the freshest project", () => {
    const stale = makeProject("stale", "/work/codework", {
      repositoryIdentity: null,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const fresh = makeProject("fresh", "/work/codework/", {
  ...
  it("uses repository identity from a duplicate registration when the winner lacks it", () => {
    const identified = makeProject("identified", "/work/codework", {
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshUnidentified = makeProject("fresh", "/work/codework/", {
      repositoryIdentity: null,
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const sibling = makeProject("sibling", "/work/codework-2");
  ...
  it("uses the freshest identity-bearing duplicate when the winner lacks identity", () => {
    ...
    const staleIdentified = makeProject("stale-identified", "/work/codework", {
      repositoryIdentity: staleIdentity,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshIdentified = makeProject("fresh-identified", "/work/codework/", {
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const winner = makeProject("winner", "/work/codework", {
      repositoryIdentity: null,
