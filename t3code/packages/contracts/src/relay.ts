...
}).annotate({ description: "Links an authenticated cloud user to a Code Work environment." });
...
>()("@codework/contracts/relay/RelayClientPrincipal") {}
...
>()("@codework/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Code Work Connect user.",
  ),
);
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Code Work Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Code Work environments, connecting authorized clients, and publishing agent activity.",
  );
