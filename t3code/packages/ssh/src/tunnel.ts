...
} from "@codework/shared/httpReadiness";
import * as NetService from "@codework/shared/Net";
import { extractJsonObject, fromLenientJson } from "@codework/shared/schemaJson";
import { satisfiesSemverRange } from "@codework/shared/semver";
...
remote_node_satisfies_engine() {
  CODEWORK_NODE_ENGINE_RANGE=@@CODEWORK_NODE_ENGINE_RANGE@@
  if [ -z "$CODEWORK_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$CODEWORK_NODE_ENGINE_RANGE" <<'NODE'
@@CODEWORK_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for CODEWORK_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$CODEWORK_NODE_BIN/node" ]; then
        PATH="$CODEWORK_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@CODEWORK_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
CODEWORK_NODE_SCRIPT_PATH=@@CODEWORK_NODE_SCRIPT_PATH@@
if [ -n "$CODEWORK_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$CODEWORK_NODE_SCRIPT_PATH" "$@"
fi
...
require_installed_t3_cli() {
  CODEWORK_CLI_PATH="$("$@" -- sh -c 'command -v t3' || true)"
  if [ -n "$CODEWORK_CLI_PATH" ]; then
    return 0
  fi
  printf 'Remote host installed %s but npm produced no t3 executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@CODEWORK_PACKAGE_SPEC@@ >&2
  return 1
}
if command -v npx >/dev/null 2>&1; then
  require_installed_t3_cli npx --yes --package @@CODEWORK_PACKAGE_SPEC@@ || exit 1
  exec npx --yes @@CODEWORK_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  require_installed_t3_cli npm exec --yes --package @@CODEWORK_PACKAGE_SPEC@@ || exit 1
  exec npm exec --yes @@CODEWORK_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the t3 CLI and could not install @@CODEWORK_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
pick_port() {
  node - "$PORT_FILE" "@@CODEWORK_DEFAULT_REMOTE_PORT@@" "@@CODEWORK_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@CODEWORK_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@CODEWORK_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@CODEWORK_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@CODEWORK_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@CODEWORK_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@CODEWORK_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env CODEWORK_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@CODEWORK_READY_TIMEOUT_MS@@"; then
    printf 'Remote Code Work server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
STATE_DIR="$HOME/.t3/ssh-launch/@@CODEWORK_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.t3"
RUNNER_FILE="$STATE_DIR/run-t3.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  ...
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      CODEWORK_PACKAGE_SPEC: packageSpec,
      CODEWORK_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
...
export function buildRemoteNodeEnvScript(input?: RemoteT3RunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      ...
      CODEWORK_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}
...
export function buildRemoteLaunchScript(input?: RemoteT3RunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    ...
    CODEWORK_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    CODEWORK_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    CODEWORK_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    CODEWORK_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}
...
>()("@codework/ssh/tunnel/SshEnvironmentManager") {
