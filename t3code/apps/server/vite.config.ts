...
export default mergeConfig(
  ...
  defineConfig({
    ...
    pack: {
      ...
      define: {
        __CODEWORK_BUILD_CHANNEL__: JSON.stringify(cliBuildChannel),
        __CODEWORK_BUILD_RELAY_URL__: JSON.stringify(repoEnv.CODEWORK_RELAY_URL?.trim() ?? ""),
        __CODEWORK_BUILD_CLERK_PUBLISHABLE_KEY__: JSON.stringify(
          repoEnv.CODEWORK_CLERK_PUBLISHABLE_KEY?.trim() ?? "",
        ),
        __CODEWORK_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: JSON.stringify(
          repoEnv.CODEWORK_CLERK_CLI_OAUTH_CLIENT_ID?.trim() ?? "",
        ),
        __CODEWORK_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: JSON.stringify(
          repoEnv.CODEWORK_RELAY_CLIENT_OTLP_TRACES_URL?.trim() ?? "",
        ),
        __CODEWORK_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: JSON.stringify(
          repoEnv.CODEWORK_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() ?? "",
        ),
        __CODEWORK_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: JSON.stringify(
          repoEnv.CODEWORK_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() ?? "",
        ),
      },
    },
    test: {
