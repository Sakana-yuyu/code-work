...
import { DEFAULT_MODEL, ThreadId } from "@codework/contracts";
...
describe("buildCodexDeveloperInstructions", () => {
  it("appends runtime info after the mode instructions", () => {
    ...
    NodeAssert.match(instructions, /Code Work/);
...
describe("Code Work browser developer instructions", () => {
  ...
  it("omits the browser block entirely when the preview tools are not attached", () => {
    for (const instructions of [
      ...
      NodeAssert.doesNotMatch(instructions, /Code Work collaborative browser/);
...
describe("hasConfiguredMcpServer", () => {
  it("detects inline Codex MCP configuration arguments", () => {
    ...
    NodeAssert.equal(
      hasConfiguredMcpServer(["-c", 'mcp_servers.code-work.url="http://127.0.0.1/mcp"']),
      true,
    );
  });
});
  ...
  it("keeps launch args when explicit app-server args are provided", () => {
    NodeAssert.deepStrictEqual(
      codexSessionAppServerArgs(
        ["-c", "mcp_servers.code-work.url=http://127.0.0.1/mcp"],
      ...
      [
        ...
        "foo",
        "-c",
        "mcp_servers.code-work.url=http://127.0.0.1/mcp",
      ],
    );
  });
});
