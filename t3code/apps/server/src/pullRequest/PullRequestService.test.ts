...
it.effect("reads nothing from a host with no implementation, but reports it", () =>
  Effect.gen(function* () {
    ...
    const service = yield* makeService({
      projects: [
        project({ id: "p1", title: "codework", workspaceRoot: "/a", repository: "Sakana-yuyu/code-work" }),
    ...
    assert.deepStrictEqual(listed, ["Sakana-yuyu/code-work"]);
      ...
      providers: [
        fakeProvider("github", {
          listChangeRequests: ({ repository }) =>
            Effect.succeed({
              items: [changeRequest(1, "2026-07-02T00:00:00Z")],
              truncated: repository === "Sakana-yuyu/code-work",
              continues: true,
            }),
        }),
      ],
...
it.effect("keeps a row already sent at the boundary instant from arriving twice", () =>
  Effect.gen(function* () {
    ...
    const result = yield* service.list({
      state: "open",
      cursors: { "github.com Sakana-yuyu/code-work": "2026-07-02T00:00:00Z|1|7" },
    });
...
it.effect("keeps the earlier exclusions when a slice ends on the instant it began on", () =>
  Effect.gen(function* () {
    ...
    const result = yield* service.list({
      state: "open",
      cursors: { "github.com Sakana-yuyu/code-work": "2026-07-02T00:00:00Z|1|6" },
    });
...
it.effect("refuses a continuation it did not issue, before asking any host anything", () =>
  Effect.gen(function* () {
    ...
    const error = yield* Effect.flip(
      service.list({ state: "open", cursors: { "github.com Sakana-yuyu/code-work": "yesterday" } }),
    );
...
it.effect("keeps healthy repositories when one of them cannot be read", () =>
  Effect.gen(function* () {
    const service = yield* makeService({
      projects: [
        project({ id: "p1", title: "codework", workspaceRoot: "/a", repository: "Sakana-yuyu/code-work" }),
        project({ id: "p2", title: "broken", workspaceRoot: "/b", repository: "Sakana-yuyu/broken" }),
      ],
      providers: [
        fakeProvider("github", {
          listChangeRequests: (input) =>
            input.repository === "Sakana-yuyu/broken"
...
it.effect("reads a host's repositories in one search, and files the rows back under each", () =>
  Effect.gen(function* () {
          ...
          listChangeRequestsAcross: (input) => {
            ...
            return Effect.succeed({
              items: [
                ...
                batchedChangeRequest(2, "Sakana-yuyu/code-work", "2026-07-02T00:00:00Z"),
              ],
              truncated: false,
            });
          },
    ...
    assert.deepStrictEqual(asked, [["Sakana-yuyu/code-work", "acme/web"]]);
