...
import { GitCommandError } from "@codework/contracts";
...
const bitbucketPullRequest = {
  ...
  links: {
    html: {
      href: "https://bitbucket.org/Sakana-yuyu/code-work/pull-requests/42",
    },
  },
  source: {
    branch: { name: "feature/source-control" },
    repository: {
      full_name: "octocat/codework",
  ...
  destination: {
    branch: { name: "main" },
    repository: {
      full_name: "Sakana-yuyu/code-work",
      workspace: { slug: "Sakana-yuyu" },
    },
  },
};
...
const repositoryJson = {
  full_name: "Sakana-yuyu/code-work",
  links: {
    html: { href: "https://bitbucket.org/Sakana-yuyu/code-work" },
    clone: [
      { name: "https", href: "https://bitbucket.org/Sakana-yuyu/code-work.git" },
      { name: "ssh", href: "git@bitbucket.org:Sakana-yuyu/code-work.git" },
    ],
  },
...
function makeLayer(input: {
  ...
  const gitMock = {
    readConfigValue: vi.fn<GitVcsDriver.GitVcsDriver["Service"]["readConfigValue"]>(() =>
      Effect.succeed<string | null>("git@bitbucket.org:Sakana-yuyu/code-work.git"),
    ),
    ...
    listRemotes: () =>
      Effect.succeed({
        remotes: [
          {
            name: "origin",
            url: "git@bitbucket.org:Sakana-yuyu/code-work.git",
            pushUrl: Option.none(),
            isPrimary: true,
          },
        ],
    ...
    Layer.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            CODEWORK_BITBUCKET_API_BASE_URL: "https://api.test.local/2.0",
            CODEWORK_BITBUCKET_EMAIL: "user@example.com",
            CODEWORK_BITBUCKET_API_TOKEN: "token",
          },
        }),
      ),
    ),
...
it.effect("parses pull request responses from the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(result, {
      number: 42,
      title: "Add Bitbucket provider",
      url: "https://bitbucket.org/Sakana-yuyu/code-work/pull-requests/42",
      baseRefName: "main",
      headRefName: "feature/source-control",
      state: "open",
      updatedAt: Option.some(DateTime.makeUnsafe("2026-01-02T00:00:00.000Z")),
      isCrossRepository: true,
      headRepositoryNameWithOwner: "octocat/codework",
      headRepositoryOwnerLogin: "octocat",
    });
    assert.strictEqual(
      execute.mock.calls[0]?.[0].url,
      "https://api.test.local/2.0/repositories/Sakana-yuyu/code-work/pullrequests/42",
    );
...
it.effect("lists pull requests with Bitbucket state and source branch query params", () => {
        ...
        values: [
          {
            ...
            source: {
              ...
              repository: { full_name: "Sakana-yuyu/code-work" },
            },
          },
        ],
      }),
  ...
  return Effect.gen(function* () {
    ...
    assert.strictEqual(
      request?.url,
      "https://api.test.local/2.0/repositories/Sakana-yuyu/code-work/pullrequests",
    );
...
it.effect("reads repository clone URLs and default branch", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(cloneUrls, {
      nameWithOwner: "Sakana-yuyu/code-work",
      url: "https://bitbucket.org/Sakana-yuyu/code-work.git",
      sshUrl: "git@bitbucket.org:Sakana-yuyu/code-work.git",
    });
  ...
  () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        [
          "https://api.test.local/2.0/repositories/Sakana-yuyu/code-work",
          "https://api.test.local/2.0/repositories/Sakana-yuyu/code-work/branching-model",
        ].toSorted(),
      );
    }).pipe(Effect.provide(layer));
  },
...
it.effect("creates repositories through the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    const request = execute.mock.calls[0]?.[0];
    assert.strictEqual(request?.url, "https://api.test.local/2.0/repositories/Sakana-yuyu/code-work");
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(JSON.parse(new TextDecoder().decode(rawBody)), {
      ...
      source: {
        ...
        repository: { full_name: "owner/codework" },
      },
      destination: {
        branch: { name: "main" },
      },
  ...
  return Effect.gen(function* () {
    ...
    yield* bitbucket.checkoutPullRequest({
      ...
      context: {
        ...
        remoteName: "origin",
        remoteUrl: "git@bitbucket.org:Sakana-yuyu/code-work.git",
      },
      reference: "42",
      force: true,
    });
...
it.effect("checks out fork pull requests through an ensured fork remote", () => {
  const { git, layer } = makeLayer({
    response: (request) => {
      if (request.url.endsWith("/repositories/octocat/codework")) {
        return Response.json({
          ...repositoryJson,
          full_name: "octocat/codework",
          links: {
            html: { href: "https://bitbucket.org/octocat/codework" },
            clone: [
              { name: "https", href: "https://bitbucket.org/octocat/codework.git" },
              { name: "ssh", href: "git@bitbucket.org:octocat/codework.git" },
            ],
          },
        });
      }
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(git.ensureRemote.mock.calls[0]?.[0], {
      ...
      url: "git@bitbucket.org:octocat/codework.git",
    });
    assert.deepStrictEqual(git.fetchRemoteBranch.mock.calls[0]?.[0], {
      cwd: "/repo",
      remoteName: "octocat",
      remoteBranch: "main",
      localBranch: "codework/pr-42/main",
    });
    assert.deepStrictEqual(git.setBranchUpstream.mock.calls[0]?.[0], {
      cwd: "/repo",
      branch: "codework/pr-42/main",
      remoteName: "octocat",
      remoteBranch: "main",
    });
    assert.deepStrictEqual(git.switchRef.mock.calls[0]?.[0], {
      cwd: "/repo",
      refName: "codework/pr-42/main",
    });
  }).pipe(Effect.provide(layer));
});
