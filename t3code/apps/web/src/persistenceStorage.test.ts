...
describe("persistenceStorage", () => {
  it("derives Code Work keys from legacy codework keys", () => {
    expect(canonicalStorageKey("codework:client-settings:v1")).toBe("codework:client-settings:v1");
  });

  it("reads canonical values without touching legacy values", () => {
    const storage = createStorage({
      "codework:settings": JSON.stringify({ version: 2 }),
      "codework:settings": JSON.stringify({ version: 1 }),
    });

    const result = readAndMigrateStorageValue({
      storage,
      canonicalKey: "codework:settings",
      legacyKey: "codework:settings",
      decode: JSON.parse,
  ...
  it("writes validated legacy values canonically before removing the legacy value", () => {
    const storage = createStorage({ "codework:settings": JSON.stringify({ version: 1 }) });
    ...
    expect(storage.values.get("codework:settings")).toBe(JSON.stringify({ version: 1 }));
    expect(storage.values.has("codework:settings")).toBe(false);
  });
  ...
  it("does not migrate malformed legacy raw values", () => {
    const storage = createStorage({ "codework:settings": "not-json" });
    ...
    expect(storage.values.has("codework:settings")).toBe(true);
  });
});
