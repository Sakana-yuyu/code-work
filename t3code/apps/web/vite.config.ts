...
import { DEV_PROXIED_PATH_PREFIXES } from "@codework/shared/devProxy";
...
const isSingleOriginDev = process.env.CODEWORK_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_CODEWORK_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.CODEWORK_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: CODEWORK_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); CODEWORK_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.CODEWORK_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // CODEWORK_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.CODEWORK_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.CODEWORK_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_CODEWORK_RELAY_URL": JSON.stringify(configuredRelayUrl),
