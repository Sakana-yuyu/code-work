    ...
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          ...
          CODEWORK_DEV_REMOTE_T3_SERVER_ENTRY_PATH: " /remote/server.mjs ",
          CODEWORK_OTLP_TRACES_URL: " http://127.0.0.1:4318/v1/traces ",
          CODEWORK_OTLP_EXPORT_INTERVAL_MS: "2500",
        },
      );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          CODEWORK_DESKTOP_APP_USER_MODEL_ID: " com.t3tools.codework.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.t3tools.codework.dev.local");
    }),
  );
