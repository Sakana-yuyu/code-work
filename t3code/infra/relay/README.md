# Code Work Connect Relay

> [!NOTE]
> Sign in to Code Work Connect from the app under Settings > Connections.

The relay is the hosted control plane for Code Work Connect. It helps clients discover and connect to
...
The relay is intentionally not in the hot path for normal Code Work traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [Code Work Connect architecture overview](../../docs/internals/code-work-connect-auth-flow.html) for the larger system
design.
...
- Linking Code Work environments to a cloud account.
...
```sh
vp run --filter codework-relay deploy
```
...
```sh
vp run --filter codework-relay deploy -- --stage prod
vp run --filter codework-relay deploy -- --env-file .env.local
```
