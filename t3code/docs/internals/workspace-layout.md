...
- `packages/contracts` (`@codework/contracts`): shared Effect Schema definitions. RPC group,
  orchestration commands/events/read model, auth scopes, environment descriptors, settings.
- `packages/shared` (`@codework/shared`): framework-agnostic utilities used by server and clients
  (`DrainableWorker`, git and source-control helpers, relay auth and signing, DPoP, semver, logging,
  observability, and more).
- `packages/client-runtime` (`@codework/client-runtime`): connection lifecycle, authorization, RPC
...
- `packages/ssh` (`@codework/ssh`): SSH config parsing, auth prompts, command execution, and the
  tunnel/environment manager behind desktop-managed SSH environments.
- `packages/tailscale` (`@codework/tailscale`): Tailscale CLI wrapper, including the
...
`@codework/shared` and `@codework/client-runtime` use explicit subpath exports with no barrel index and
no root export. Import the narrow path (`@codework/shared/DrainableWorker`,
`@codework/client-runtime/state/threads`) rather than the package root. Files that are not exported
are implementation details. `@codework/contracts` does export a root alongside `./settings` and
`./relay`.
