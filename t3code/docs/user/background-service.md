# Running Code Work in the Background

On Linux and macOS, Code Work can run as a background service for your user, so it is ready without
...
The service runs a small stable launcher. Exact Code Work versions are installed separately, so a
...
**Linux** uses a systemd user unit at `~/.config/systemd/user/codework.service`. The service starts
when the machine boots and keeps running after you log out (lingering is enabled during install).

**macOS** uses a launch agent at `~/Library/LaunchAgents/com.t3tools.codework.service.plist`. It
...
## Using It with Code Work Connect
