...
Code Work connects to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving the app.

## Supported Providers

Code Work works with the platforms your team already uses:
...
- Code Work can suggest titles and descriptions based on your commits
...
- Command-click (Control-click on Windows and Linux) a pull request number in the sidebar to open it in your browser instead of in Code Work
...
1. Install the GitHub CLI (version 2.81.0 or newer) on the machine running Code Work:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in Code Work and verify GitHub shows as authenticated
...
machine running Code Work.

Recommended, a Bitbucket access token:

```bash
export CODEWORK_BITBUCKET_ACCESS_TOKEN="your-access-token"
```
...
```bash
export CODEWORK_BITBUCKET_EMAIL="you@example.com"
export CODEWORK_BITBUCKET_API_TOKEN="your-token"
```

If both are set, the access token wins. Restart Code Work and verify the connection in **Source
Control settings**.
...
- **GitHub says it could not verify sign-in status** – Code Work needs GitHub CLI 2.81.0 or newer to check sign-in status. Update `gh` (e.g., `brew upgrade gh`), then rescan
