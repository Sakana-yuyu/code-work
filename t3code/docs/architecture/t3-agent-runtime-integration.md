# Code Work Agent Runtime 集成决策

## 目的

本文是 Code Work 统一 IDE、Agent Driver 和多 Agent Runtime 的架构入口。目标是让 Code Work 保持独立可用的 IDE 壳，同时把 Provider、BYOK、ACP、CLI、MCP、Cursor/VSCode Adapter 和 Multica 接入同一个任务、权限和工具执行平面。
...
- Multica daemon 的注册、心跳、quick-create、任务状态投影、Leader/Squad 显式路由和 Code Work Task Graph 元数据传递。
...
### `cursor-byok` 可吸收，但需要 Code Work 原生重做

- 多协议 BYOK 请求和 Provider 错误归一化：映射到 Code Work Provider/BYOK adapter，不复制 Cursor 私有消息。
...
- Skills、原生提示词和 sparse activation：复用 Code Work Skill Registry 与 prompt 编译边界，逐项补充合同和 UI。
- Request/Runtime diagnostics：使用 Code Work 的 runtime、task、run、exchange 标识和统一脱敏格式重做。
...
这些能力依赖 Cursor 私有客户端生命周期或本机协议，不能成为 Code Work Provider、Composition、ToolBroker 或原生 IDE 的前置依赖。若实现，必须有独立 Runtime ID、配置、进程生命周期、日志、审计和回滚边界。
...
- Multica 在 Code Work 壳内通过真实 daemon 执行 Leader -> Squad -> Task Graph -> 子 Agent -> 结果汇聚 -> Review 的
  完整外部编排链仍未验收；当前本地 Task Graph 可使用 Code Work Driver/ToolBroker，外部 Multica 仍受窄协议限制。
- Multica 官方窄协议之外的 Tool-call/Grant handshake。Code Work 现在已经提供独立的版本化 Tool Bridge HTTP 合同和客户端，但没有证据证明官方 daemon 原生接受 Code Work grant 或能够回调该 Bridge；因此只有显式安装/配置扩展并完成握手后才能开放工具权限。
...
Multica 作为外部执行 Runtime 接入 Code Work，而不是替代 Code Work 的任务和权限控制面。

```text
Code Work Task
  -> Code Work Leader / Squad / Task Graph
  -> Multica quick-create 或本地执行 route
  -> daemon claim / start / progress / complete / fail
  -> Code Work event projector
  -> Leader 汇聚结果
  -> Code Work review checkpoint
```

必须遵守以下边界：

1. Code Work 的 `taskId`、`runId`、`agentId`、`squadId`、Multica `task_id`、远端 Agent/Squad UUID 显式映射，禁止按名称猜测。
2. quick-create 返回异步 `task_id` 只能证明任务进入队列，不能直接推断完成。
3. WebSocket 唤醒帧不是终态事实；终态必须来自 HTTP 状态或带稳定事件 ID 的任务事件。
4. 当前官方窄协议没有可验证的 Code Work Capability Grant 接收、Tool-call RPC 或 revoke 协议。未配置扩展时，带 grant 的外部派发必须返回明确的 `capability_handshake_unsupported`，不能把 grant 引用当作授权完成。
5. Code Work 已提供 `t3-composition-runtime/1` Tool Bridge：外部 Runtime 使用 `POST /api/composition/runtime/tools/invoke` 和 `/cancel`，请求必须带完整 Task/Run/Agent/Handshake/Grant/幂等身份；服务端再次校验 scope 后才进入 Code Work ToolBroker。Multica 只有在显式 capability extension 接受同一组 grant 后才能使用这条路径。
6. 若要让 Multica 使用 Code Work canonical tools，优先通过显式 MCP bridge 或经过审计的独立 daemon extension；bridge 必须重新校验 `taskId`、`runId`、`agentId`、`grantId` 和过期时间，并最终进入 Code Work ToolBroker。
...
3. Code Work 原生 Leader/Squad/Task Graph 执行器，包含并行、依赖、隔离、取消、超时、重试、结果汇聚和 review。
4. Multica bridge/extension 协议协商；当前已落地 Code Work Tool Bridge 合同，下一步仍需真实 daemon extension 和跨进程 E2E，官方窄协议未实现时保持稳定拒绝。
5. Cursor/VSCode 独立 Adapter，先实现 Code Work IDE API 的可验证 operation，再接真实 transport。
