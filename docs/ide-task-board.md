...
| SAFE-003 | 进行中 | 建立 IDE 审批状态机 | SAFE-002 | 写文件、clone、Git mutation、Agent effect 和外部 executor write 都要求单次、过期、可取消审批 | `internal/ide/approval` receipt state machine 定向测试通过；尚未由 effect coordinator 消费 |
...
| IDE-004 | 未开始 | 编辑保存与 diff 审批 | IDE-003, SAFE-003 | ETag/版本冲突可见；用户写入经预览/审批；无静默覆盖 | — |

## Git 与 SSH

| ID | 状态 | 工作项 | 依赖 | 验收标准 | 完成证据 |
| --- | --- | --- | --- | --- | --- |
| GIT-001 | 未开始 | 只读 Git 状态、分支、diff 与 remote 摘要 | IDE-001 | 使用系统 Git typed argv；不执行 raw shell；remote URL 不泄密 | — |
| SSH-001 | 未开始 | 应用管理 SSH 私钥 vault | SAFE-003 | 私钥 DPAPI 加密，普通 DTO/日志/前端绝不含私钥或口令 | — |
