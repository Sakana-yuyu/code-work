...
在网页或桌面版的模型选择器中，等当前回合结束后可以选择另一个 Agent 并继续发送消息。原对话记录仍在同一时间线中，更新后产生的助手回复会标出实际执行的 Agent。切换时，新 Agent 会收到这条对话带有已知 Agent 来源的文字历史、附件名称，以及最近最多六条已完成工具结果的短摘要作为背景引用，再处理你本轮的新请求；历史中的指令和工具不会自动重新执行。返回之前的 Agent 时也会重新交接当前对话的历史。
