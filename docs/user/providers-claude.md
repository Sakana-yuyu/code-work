...
This guide is for people who want to use more than one Claude setup in Code Work. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Code Work](./install.md).
...
In Code Work Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Code Work uses Claude Code's normal config directory.

When you set this field, Code Work points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, CodexWork
...
Code Work looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where Code Work looks.

Then add another Claude provider in Code Work:
...
Code Work only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Code Work keeps separate config directories isolated
...
Add or edit a Claude provider in Code Work Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Code Work stores the value as a server secret and does not
...
Code Work does not need a special Claude Code Router provider. Treat the router as a Claude
