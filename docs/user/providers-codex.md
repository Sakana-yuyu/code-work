...
This guide is for people who want to use more than one Codex account in Code Work. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Code Work](./install.md).
...
“已禁用”表示 Codex 连接未启用，不代表本机没有安装 CLI。启用后会检测程序和登录状态；仅当检测确认 CLI 缺失或安装损坏（例如 npm 丢失了平台二进制包），且当前平台支持自动安装时，才显示“安装 CLI”或“修复 CLI”入口。检测尚未完成时不会提示重新安装。
...
issue. Code Work uploads the thread and Codex logs to OpenAI and shows a thread ID that you can copy
and share with OpenAI employees.

## Approve access to other apps

When a Codex tool needs access to an app such as Safari, Code Work shows the app name and asks for
...
- both accounts can see the same Code Work/Codex sessions
...
In Code Work Settings, name it something obvious:
...
In Code Work Settings, add another Codex provider:
...
Code Work shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Code Work treats it
