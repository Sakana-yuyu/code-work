...
Code Work Connect uses one Clerk application for web, desktop, and mobile authentication. The relay verifies
two kinds of bearer credential: template JWTs generated from the `t3-relay` template with the shared
`code-work-relay` audience, and Clerk OAuth tokens issued to the CLI. `verifyRelayClientBearerToken` in
...
[code-work-connect-auth-flow.html](./code-work-connect-auth-flow.html).

## Application Keys

Code Work Connect is disabled in a fresh clone. To enable it for source builds against the production
...
```dotenv
CODEWORK_CLERK_PUBLISHABLE_KEY=<publishable key>
CODEWORK_CLERK_JWT_TEMPLATE=<JWT template name>
CODEWORK_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
CODEWORK_RELAY_URL=https://relay.example.com
```
...
should set `CODEWORK_CLERK_PUBLISHABLE_KEY`, `CODEWORK_CLERK_JWT_TEMPLATE`,
`CODEWORK_CLERK_CLI_OAUTH_CLIENT_ID`, and `CODEWORK_RELAY_URL` before building. EAS preview and
production builds only need the Clerk publishable key, JWT template name, and relay URL in their EAS
environment.

When any client-facing public value is absent, cloud UI is omitted. The `codework connect` command group is
...
deployment defaults.
`vp run --filter codework-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `codework connect` commands authorize a headless environment with a separate Clerk OAuth application.
This uses an OAuth public client with PKCE, so the CLI stores no client secret.

In **Clerk Dashboard > OAuth applications**:

1. Create an OAuth application for the Code Work CLI.
...
3. Add **both** allowed redirect URIs:
   ...
   - `https://app.t3.codes/connect/callback` for the hosted out-of-band flow. This is
     ...
     custom `CODEWORK_HOSTED_APP_URL` means `$CODEWORK_HOSTED_APP_URL/connect/callback` instead.
     Omitting it breaks headless and SSH authorization.
4. Enable the `openid`, `profile`, and `email` scopes.
5. Set `CODEWORK_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
```sh
codework connect            # default: onboarding
codework connect login
codework connect link       # --publish-only
codework connect status     # --json
codework connect publish    # --disable
codework connect unlink
codework connect logout
```

`codework serve` is a separate top-level command, not a connect subcommand.

`codework connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `codework connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running Code Work server. The next `codework serve` or `codework start` reconciles the relay link and launches the
managed tunnel. `codework connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `codework connect link` can re-enable exposure without another browser flow. `codework connect
...
logout leaves it running; manage it with `codework service status`, `install`, `update`, and `uninstall`.
...
`CLERK_JWT_AUDIENCE=code-work-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `CODEWORK_RELAY_URL` still
...
Local desktop development uses `codework-dev://app`, while packaged builds use `codework://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `codework-dev://app`; the production Clerk instance should only need
`codework://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["codework://app"]}'
```
...
The production macOS bundle ID is `com.t3tools.codework`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.t3tools.codework` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.t3tools.codework`.
...
```dotenv
CODEWORK_APPLE_TEAM_ID=ABC1234567
CODEWORK_MACOS_PROVISIONING_PROFILE=/absolute/path/to/codework.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
CODEWORK_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `CODEWORK_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`CODEWORK_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `CODEWORK_PORT` set. Rebuild the signed app
