...
每个请求重新读取服务器设置，因此模型通道的 URL、密钥和模型修改无需重启网关。上游响应流默认保持原样。`gatewayAnthropicBridge.ts` 将落到 openai 通道的 Anthropic Messages 请求翻译为 Chat Completions，并把 SSE 翻译回 Messages（含工具调用和用量）；`count_tokens` 返回估算值。`gatewayResponsesBridge.ts` 仅在普通 openai 中转的 Responses 端点返回 404、405 或 501 时，将可表达的文本、图片 URL、函数及自定义工具请求改发同一通道的 Chat Completions，并把 JSON 或 SSE 转回 Responses；原生成功线路不受影响。本地官方账号保持原生协议，不做此回退。带 `previous_response_id`、持久会话标识、未知工具或无法表达的内容会得到明确 422，避免静默丢失上下文；Responses → Anthropic Messages 仍不支持。`openai` 是通道协议分类，不是上游同时支持 Responses 与 Chat Completions 的能力证明。中转通道（非本地账号池）在响应流交付 CLI 之前，对上游 5xx 与传输失败最多重试到 `RELAY_GATEWAY_MAX_ATTEMPTS` 次，退避取上游 `retry-after`（封顶 5 秒）或指数回退；429 原样透传给 CLI 自行退避。

协议桥参考（检索日期 2026-09-24）：关键词 `OpenAI Responses function_call_output streaming events chat completions`、`cc-switch responses_to_chat_completions`；采用 [OpenAI Responses API](https://developers.openai.com/api/reference/typescript/resources/responses/methods/create)、[OpenAI Function Calling](https://developers.openai.com/api/docs/guides/function-calling) 与 [cc-switch 转换实现](https://github.com/farion1231/cc-switch/blob/main/src-tauri/src/proxy/providers/transform_codex_chat.rs)。前两者定义请求、工具及流式事件形状，后者用于核对 Codex 对 Chat 中转的适配边界；实现仍沿用 Code Work 的 Effect HTTP、凭据和日志链路。
