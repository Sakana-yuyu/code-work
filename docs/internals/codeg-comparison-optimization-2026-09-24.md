...
| 候选项           | 当前结果                                                                                                                                        | 尚未覆盖的边界                                                                                                                      |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| 1. 外部 CLI 会话 | 已实现 Codex 与 Claude Code 历史扫描、只读导入、稳定去重、来源提示，以及原 Provider 就绪时的续聊入口。                                          | 尚未实测有认证的原生会话续聊；其他 Provider 日志格式与附件未接入。                                                                  |
| 2. ACP 目录      | 已接入官方目录、环境平台筛选、版本一致时的固定 npx 命令预填、实例状态诊断；非零探测给出安全错误提示，明确不支持 `--version` 时保留握手检查。    | 尚无独立的包安装检测或二进制校验与安装；未配置实例不等于包未安装。                                                                  |
| 3. 任务看板      | 已有待办、运行、待处理、完成分栏、本地待办、并发上限和并行工作目录校验；现在可从持久化执行输入关联任务工作目录，并按需查看只读 Git 状态与差异。 | 任务与线程没有普遍可靠的关联；差异预览不等于 Git 接收核验，仍需人工核对目标分支及合并结果。目录别名和运行时并发租约仍需进一步约束。 |
| 4. 对话分屏      | 支持三个对话分栏、横纵切换、关闭递补、第二与第三栏交换，以及本机布局持久化；草稿沿用原存储。                                                    | 主栏由当前路由决定，仍不能任意移动；三个活动时间线尚未做性能基准。                                                                  |
| 5. Office 预览   | `.docx`、`.xlsx`、`.pptx` 具备受授权只读预览、刷新和变更检查；Excel 显示工作表名称、行列与截断提示。用三种真实生成文件完成解析验证。            | 不能忠实还原排版、公式格式、图表和图片；真实复杂文件的交互验收仍需补充。                                                            |
| 6. 跨会话引用    | 已可在已连接且有读取权限的环境中选择线程，预览有限片段，标注来源并插入当前草稿。                                                                | 不自动概括完整历史，也不将引用内容视为可信指令。                                                                                    |
| 7. 外部消息渠道  | 已完成需求与风险评估；按用户选择暂不接入。                                                                                                      | 待需求量与渠道成本数据明确后再决定是否启动设计。                                                                                    |

上述实现位于独立工作树，未合并、推送或部署。原 Code Work 工作目录中的现有修改保持不变。

本轮 ACP 元数据核对检索词为 `ACP registry agent.schema.json distribution npx version`，访问日期为 2026-09-24；采用 [ACP 官方注册表 Schema](https://github.com/agentclientprotocol/registry/blob/main/agent.schema.json)和[官方投稿说明](https://github.com/agentclientprotocol/registry/blob/main/CONTRIBUTING.md)，因为两者直接定义目录字段与固定版本约束。Excel 解析核对检索词为 `SpreadsheetML workbook relationship sheet shared string inline string`，同日采用 [Microsoft Open XML 工作簿结构说明](https://learn.microsoft.com/en-us/office/open-xml/spreadsheet/structure-of-a-spreadsheetml-document)和[工作表说明](https://learn.microsoft.com/en-us/office/open-xml/spreadsheet/working-with-sheets)，因为它们直接说明工作表关系和单元格结构。
