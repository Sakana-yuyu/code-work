...
Code Work uses [Vite+](https://viteplus.dev/guide/). Install the global `vp` command, install
...
- `vp run dev --share`: Also publishes the web port over HTTPS on this machine's tailnet. The
  ...
  Shared runs default to Vite's bundled dev mode (`CODEWORK_BUNDLED_DEV=1`): a remote browser pays a
  network round trip per import level in unbundled dev, which turns a cold module graph into
  minutes of waterfall. Set `CODEWORK_BUNDLED_DEV=0` to opt a shared run back out.
- `vp run dev --browser`: Auto-opens a browser. Off by default. The dev runner writes
  `CODEWORK_NO_BROWSER` itself from this flag, so setting `CODEWORK_NO_BROWSER=0` in your environment has
...
- Pass dev-runner flags directly after the root task name, for example:
  `vp run dev --home-dir /tmp/codework-dev`

### Dev state directories

- Dev commands run from a linked **git worktree** default to that worktree's gitignored `.t3`, even
  when `CODEWORK_HOME` is set, storing state in `<worktree>/.t3/userdata`. Pass `--home-dir <path>` to
...
- `vp run build`: Fans out over `apps/*`, `packages/*`, `oxlint-plugin-codework`, and `scripts`.
...
- Desktop production windows load the bundled UI from the `codework://app/` root URL (not a
  `127.0.0.1` document URL, and not an explicit `index.html` path).
- Desktop packaging includes `apps/server/dist` (the `codework` backend) and starts it on loopback with an
...
- Signed macOS builds also require `CODEWORK_APPLE_TEAM_ID` and
  `CODEWORK_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `CODEWORK_CLERK_PUBLISHABLE_KEY` unless `CODEWORK_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
- Example: `CODEWORK_DEV_INSTANCE=branch-a vp run dev:desktop`

Offset resolution, in order:

1. `CODEWORK_PORT_OFFSET`, which must be a non-negative integer. Negative values are rejected.
2. `CODEWORK_DEV_INSTANCE`. An all-digit value is used directly as the offset; any other non-empty
