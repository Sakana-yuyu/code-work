...
### 2026-09-05：单节点选择与输入框胶囊

Web/Desktop 的加号菜单展开全部 17 个独立入口；Mobile 的加号菜单同时保留附件入口和节点子菜单。
选择节点后使用原能力更新 RPC 保存 `selectedIntent`，仅在保存成功后显示所选胶囊；胶囊支持更换节点和停用。
节点是线程级持久选择，下一次发送时生效。Migration 077 为旧记录填入 `workflow`，旧客户端省略该字段时保留已有选择。

Provider 只注入所选节点的职责和合法回传指令。`chat/status/stash/resume` 在没有工作流时不初始化产物。
路由和服务层拒绝未选中的节点；单节点 Task 完成后 Reactor 不自动派发下一阶段。
文档节点通过 `complete-node` 记录完成，可以独立重做调研、澄清、设计、方案和修订；
该转换清除旧批准和验收结果，方案/修订完成后进入人工批准。运行中的任务、暂停状态和未决需求仍由服务端校验。
明确选择 `verify/ship` 时寻找独立执行者；`loop` 使用界面明确提示的 3 轮预算。
运行中的 Loop 订阅原能力事件；移除胶囊或切换到其他节点后，下一轮检查取消，不继续启动迭代。

回滚方式：移除胶囊即将当前线程能力设为关闭；保留既有产物和已执行任务，运行中任务由原停止入口处理。
数据库迁移只增加列，旧版本忽略该列即可读取原开关，不需要删除用户数据。

本次验证记录（与下文历史 A1-A8 验收分开）：

- 定向测试共 14 个文件、63 个用例通过；包含节点持久化与迁移、单节点路由/完成门禁、禁止串联、Loop 暂停/停用/切换取消和中英日节点标签。
- Server、Contracts、Web、Mobile 定向类型检查通过，Web 生产构建通过；没有执行仓库全量检查。
- 经用户授权，在临时目录的隔离环境完成真实 Web 验收：加号展开 17 个节点、选择技术设计显示胶囊、刷新保留、点击胶囊切换到讨论、移除胶囊和 390px 窄屏交互。
...
- 明确选择“完整流程”后，Agent 根据当前工作流状态、项目产物和用户意图，自动选择合适阶段。
- 单节点选择限定本次能力范围；前置条件不足时说明原因和建议，不擅自启动其他节点。完整流程仍按合法前置条件纠偏。
...
原项目的 16 个命令保留为能力节点，加上产品内建的 `acceptance`（检查验收），共 17 个可选入口，集中放在“+ → 规格工作流”的节点菜单中。
...
| acceptance | 检查验收           | 检查验收条件，最终完成仍由用户确认 |
...
当前 Server 的终态反应已经落地为 `SpecWorkflowReactor`：完整流程的 `apply` 成功后，只有持久化的
独立验证者身份存在、工作流仍 active 且能力仍 enabled 时才自动派发 `verify`；`verify`
成功后只自动进入 `acceptance`，不会替用户完成最终验收。暂停、失败、取消、超时或能力关闭
都会停止后续自动唤醒，并保留可重试的阶段状态。单独选择的节点完成后不串联下一阶段。
...
- `ProviderCommandReactor` 只有在当前线程 capability 已确认 `enabled=true` 时，才注入所选节点协议；需要工作流状态的节点才初始化 state。未建立工作流时的讨论、进度、暂停和恢复不会初始化产物。
...
- `verify` 不由同一自然语言回合直接伪造独立身份；完整流程由实施 Task 成功后的 Reactor 唤醒，明确选中 `verify/ship` 时则由 Server 寻找独立执行者。低置信度、普通讨论和无明确动作不应返回 marker。
...
完整流程的纠偏示例（单节点模式仅说明缺失条件，不自动执行其他节点）：
...
菜单保留一个内建分组，展开后可选择全部节点；点击分组本身不会启用整套流程：

```text
规格工作流 → 完整流程 / 调研与约束 / 澄清需求 / 讨论想法 / 技术设计 / …
```

选择成功后输入框显示可切换、可移除的胶囊，下一次发送时执行所选部分：

```text
[ 规格工作流 · 技术设计  × ]
点击胶囊：更换节点，查看当前阶段及以下适用操作
- 方案确认：批准 / 拒绝
- 最终验收：完成最终验收
- 工作流：暂停 / 恢复
```
