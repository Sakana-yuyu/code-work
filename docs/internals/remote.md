...
Code Work has one runtime boundary: a client talks to a Code Work server over HTTP and WebSocket, and the server
...
└───────────────┬──────────────────────────────┘
                │ connects to one Code Work server
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Code Work server        │
...
One running Code Work server instance. It owns provider availability and auth, model availability, projects
...
| `RelayConnectionTarget`   | Managed Code Work Connect relay tunnels.                                        |
...
Tailscale is the first provider, and Code Work manages more than discovery. When `tailscaleServeEnabled` is
...
(`apps/server/src/server.ts`, using [`@codework/tailscale`](../../packages/tailscale/src/tailscale.ts)).
...
Access answers one question: how does the client speak WebSocket to a Code Work server? It does not answer
...
Managed Code Work Connect relay tunnels use `RelayConnectionTarget` and are the answer when the host is
...
Worker itself. See [connect.md](./connect.md).

### Tailscale access

A Code Work-managed `tailscale serve` mapping exposes the server on the tailnet over HTTPS, and the
...
[packages/ssh/src/tunnel.ts][sshtunnel], which resolves the target, launches or reuses the remote Code Work
...
Launch answers a different question: how does a Code Work server come to exist on the target machine? Keep
it separate from access.

- **Pre-existing server.** The operator already runs Code Work and the client connects directly or through a
  tunnel.
...
- a relay-hosted OAuth callback broker (see [connect.md](./connect.md));
