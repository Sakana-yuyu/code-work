...
The screenshot harness runs the real mobile application against three disposable local Code Work
...
the software keyboard, and supplies deterministic Code Work Connect discovery rows to the real
...
1. Creates three temporary Code Work base directories and starts a local server for each on an available
   port.
2. Creates Code Work, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic Code Work review diff.
...
Every configured device defaults to dark appearance and the `code-work` palette, so plain
...
Pass `--theme <id>` (repeatable) or `--theme all` to capture the app's other palettes: `code-work`,
...
| `google-play/phone/dark/code-work/`     | Pixel AVD at 420 dpi      | 1080×1920         | Google Play phone, portrait 9:16          |
| `google-play/tablet-7/dark/code-work/`  | Pixel AVD at 600dp width  | 1080×1920         | Google Play 7-inch tablet, portrait 9:16  |
| `google-play/tablet-10/dark/code-work/` | Pixel AVD at 800dp width  | 1440×2560         | Google Play 10-inch tablet, portrait 9:16 |
...
The generated tree is deliberately aligned with the store upload fields:
    ...
    ├── apple/
    │   ├── iphone-6.9/dark/code-work/{thread,terminal,review,threads,environments}.png
    │   ├── iphone-6.5/dark/code-work/{thread,terminal,review,threads,environments}.png
    │   └── ipad-13/dark/code-work/{thread,terminal,review,threads,environments}.png
    └── google-play/
        ├── phone/dark/code-work/{thread,terminal,review,threads,environments}.png
        ├── tablet-7/dark/code-work/{thread,terminal,review,threads,environments}.png
        └── tablet-10/dark/code-work/{thread,terminal,review,threads,environments}.png

A light-only run writes the same tree under `light/`; `--appearance both` writes both appearance
folders, and each requested theme adds a sibling folder next to `code-work/`.
...
ARM64 by default for local Apple Silicon development; CI sets `CODEWORK_SHOWCASE_ANDROID_ABI=x86_64` so the
...
`pnpm --filter @codework/mobile showcase` starts Expo on its normal port, so it is not compatible with
...
The Pending rows use the production offline outbox and point at the real Code Work and React fixture
