...
1. Production 构建：最新 production 构建的 appVersion 与 `app.config.ts` 的 `version` 不同（或不存在）时，`eas build --profile production --no-wait` 调度新构建。不上架、不提交商店；产物（APK/AAB/IPA）到 EAS 控制台下载。
...
- 如将来要恢复自动上架（TestFlight / Play 内部 track），把 `--auto-submit` 加回两处 `eas build`，并先在 EAS 控制台配好 Apple API Key 与 Google Play 服务账号 JSON。
