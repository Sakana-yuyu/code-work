...
// Claude Code / Codex / Gemini / Copilot / Cline / ZCode / 共享 .agents / Code Work）的
...
const (
	...
	SkillSourceCodeWork      SkillSource = "code-work"      // Code Work 自己的应用数据技能目录
)
...
// Claude → Codex → Gemini → Copilot → Cline → 共享 .agents → ZCode → ZCode 插件 → Code Work。
func orderedSkillScanRoots(workspaceRoot string) []skillScanRoot {
	...
	// Code Work 自己的应用数据目录；不读取 Cursor BYOK 的历史数据目录。
	add(appdata.SkillsRootPath(), SkillSourceCodeWork)
