...
// 在其基础上补齐 Code Work 自己的技能根目录里多源扫描未覆盖的技能。
...
func scanSkillsForWorkspace(workspaceRoot string, snapshot skillStoreScanSnapshot) ([]GlobalSkill, error) {
	...
	// Code Work root 作为保底补充（可能有多源扫描未纳入的目录布局）。
	if strings.TrimSpace(snapshot.root) != "" && sourceEnabled(snapshot.skillSources, string(SkillSourceCodeWork)) {
