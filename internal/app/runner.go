...
func Run(resources EmbeddedResources) error {
	...
	app.Event.OnApplicationEvent(events.Common.ApplicationStarted, func(event *application.ApplicationEvent) {
		logger.Infof("Code Work version: v%s", buildinfo.CurrentVersion())
		updateManager.Start()
		startAdRefreshLoop(adRefreshCtx)
		// A fresh Code Work installation must not silently take over an existing
		// Cursor profile. Starting the local proxy or writing bundled skills now
		// requires an explicit action from the user through the UI or tray menu.
		logger.Infof("local service remains stopped until the user explicitly starts it")
	})
