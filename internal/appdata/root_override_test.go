...
const (
	cursorBYOKAppDirName       = ".cursor-local-assistant-v2"
	cursorBYOKLegacyAppDirName = ".cursor-local-assistant"
)
...
func TestRootDirHonorsCodeWorkHomeOverride(t *testing.T) {
	override := t.TempDir()
...
func TestCodeWorkRootNeverOverlapsCursorBYOKRoots(t *testing.T) {
	override := t.TempDir()
	t.Setenv(RootDirEnvVar, override)
	...
	for name, got := range map[string]string{
		...
		for _, oldRoot := range []string{
			filepath.Join(override, cursorBYOKAppDirName),
			filepath.Join(override, cursorBYOKLegacyAppDirName),
		} {
			if isInside(oldRoot, got) {
				t.Fatalf("%s() = %q，落在旧 Cursor BYOK 数据目录 %q 内", name, got, oldRoot)
			}
		}
	}
}
...
func TestRootDirUnderTestNeverResolvesInsideRealUserHome(t *testing.T) {
	...
	for name, got := range map[string]string{
		...
		if isInside(productionRoot, got) || isInside(filepath.Join(realHome, cursorBYOKAppDirName), got) || isInside(filepath.Join(realHome, cursorBYOKLegacyAppDirName), got) {
			t.Fatalf("%s() = %q，落在真实用户数据目录 %q 内；测试进程绝不允许解析到真实 appdata 根目录", name, got, realHome)
		}
	}
}
