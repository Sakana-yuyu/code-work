...
>
> 进度已按实际代码同步（2026-08-27）：各项以 `t3code` 仓库提交为证据（见各项标注）。
> 本清单范围之外的后续批次（Batch A~F2：MCP、IDE 门、Multica、Provider ToolBroker 等）
> 见 `../../../t3code/spec/changes/t3-multica-runtime/design.md` 的逐批落地记录；
...
- [x] 1. Shared composition contracts
  - [x] 1.1 Add capability, model tool call, ToolBroker, AgentDriver, IDE descriptor, Task/Run/Event schemas and stable error codes. (evidence: t3code `packages/contracts/src/composition.ts`、`compositionRuntime.ts`)
...
- [x] 3. BYOK Agent Loop
  - [x] 3.1 Extract a protocol-neutral model-run state machine from the existing BYOK adapter. owner: backend deps: 1.1 (evidence: t3code 654c0a2f，`apps/server/src/composition/ByokAgentLoop.ts`)
...
- [x] 5. Integration and acceptance
  ...
  - [x] 5.3 Record excluded Cursor/Multica/Squad capabilities and confirm the old BYOK pure-text, Workspace/MCP and Checkpoint paths remain available. deps: 5.2 (evidence: `t3code/docs/internals/byok-multica-migration-matrix.md` 记录未迁移能力域；各批回归确认旧路径保留)
