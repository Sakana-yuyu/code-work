// @effect-diagnostics nodeBuiltinImport:off globalTimers:off globalDate:off - This host-side fixture creates an isolated local Code Work environment.
...
export const SHOWCASE_PROJECT_ID = "codework";
...
const PROJECT_FAVICONS = {
  codework: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
...
export const SHOWCASE_PROJECTS = [
  {
    ...
    title: "Code Work",
    directory: "codework",
    repositoryUrl: "https://github.com/Sakana-yuyu/code-work.git",
    favicon: PROJECT_FAVICONS.codework,
  },
  {
    id: "react",
    title: "React",
...
async function seedCodeworkWorkspace(workspaceRoot: string): Promise<void> {
  ...
  await NodeFSP.writeFile(
    ...
    `${JSON.stringify({ name: "codework", private: true, scripts: { test: "vp test" } }, null, 2)}\n`,
  );
  await NodeFSP.writeFile(NodePath.join(workspaceRoot, "favicon.svg"), PROJECT_FAVICONS.codework);
  await NodeFSP.writeFile(
...
async function seedCompanionWorkspace(input: {
  ...
  await NodeFSP.writeFile(
    ...
    `# ${input.title}\n\nSeeded by the Code Work mobile screenshot harness.\n`,
  );
  await initializeRepository({
