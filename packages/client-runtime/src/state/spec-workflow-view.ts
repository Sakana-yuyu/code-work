...
export interface SpecWorkflowViewInput<R, E> {
  /** The app's connection atom runtime. */
  readonly runtime: Atom.AtomRuntime<EnvironmentRegistry | R, E>;
...
/**
 * One wiring point for the spec-workflow view shared by web and mobile.
 * Only the Composer's explicit toggle writes capability; ordinary messages
 * never fire that command.
 */
export function createSpecWorkflowView<R, E>(input: SpecWorkflowViewInput<R, E>) {
  const specWorkflowEnvironment = createEnvironmentSpecWorkflowAtoms(input.runtime);

  const EMPTY_CAPABILITY_RESULT_ATOM = Atom.make(
    AsyncResult.initial<SpecWorkflowCapability, never>(false),
  ).pipe(Atom.withLabel(`${input.labelPrefix}-spec-workflow:empty`));
  const EMPTY_CAPABILITY_EVENT_ATOM = Atom.make(
    AsyncResult.initial<SpecWorkflowEvent, never>(false),
  ).pipe(Atom.withLabel(`${input.labelPrefix}-spec-workflow-events:empty`));
  const EMPTY_STATE_RESULT_ATOM = Atom.make(
    AsyncResult.success<SpecWorkflowState | null, never>(null),
  ).pipe(Atom.withLabel(`${input.labelPrefix}-spec-workflow-state:empty`));
  const EMPTY_STATE_EVENT_ATOM = Atom.make(
    AsyncResult.initial<SpecWorkflowStateEvent, never>(false),
  ).pipe(Atom.withLabel(`${input.labelPrefix}-spec-workflow-state-events:empty`));

  function useSpecWorkflowCapability(
  ...
  function useSpecWorkflowController(
    threadRef: ScopedThreadRef | null,
  ): SpecWorkflowCapabilityController {
    const state = useSpecWorkflowCapability(threadRef);
    const workflowState = useSpecWorkflowState(
      state.capability?.enabled === true ? threadRef : null,
    );
  ...
  return {
    specWorkflowEnvironment,
    useSpecWorkflowCapability,
    useSpecWorkflowState,
