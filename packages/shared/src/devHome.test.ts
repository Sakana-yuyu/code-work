...
import { resolveGitWorktreePath, resolveWorktreeCodeworkHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "codework-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeCodeworkHome", () => {
  it.effect("answers with .t3 before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeCodeworkHome(nested);
