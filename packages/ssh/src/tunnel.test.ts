...
describe("ssh tunnel scripts", () => {
  it("builds the remote t3 runner with npx and npm fallbacks", () => {
    ...
    assert.include(script, `CODEWORK_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for CODEWORK_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
  ...
  it("builds the remote t3 runner with a node script override", () => {
    ...
    assert.include(
      script,
      "CODEWORK_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$CODEWORK_NODE_SCRIPT_PATH" "$@"');
  });
  ...
  it("uses the remote t3 runner for launch and pairing scripts", () => {
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `CODEWORK_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote Code Work server did not become ready");
