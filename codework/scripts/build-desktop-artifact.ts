...
const DESKTOP_PROTOCOLS = [
  {
    ...
    schemes: [
      ...
      ...PRODUCT_IDENTITY.legacySchemes,
    ],
  },
] as const;
...
interface StagePackageJson {
  ...
  readonly codeworkCommitHash: string;
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // Code Work always passes the user's installed Claude executable to the SDK,
...
export class InvalidAppleTeamIdError extends Schema.TaggedErrorClass<InvalidAppleTeamIdError>()(
  ...
  override get message(): string {
    return `CODEWORK_APPLE_TEAM_ID '${this.teamId}' must be a 10-character Apple Developer Team ID.`;
  }
}
...
export class MissingMacPasskeyProvisioningProfileError extends Schema.TaggedErrorClass<MissingMacPasskeyProvisioningProfileError>()(
  ...
  override get message(): string {
    return "CODEWORK_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.";
  }
}
...
export class MissingMacPasskeyDomainConfigurationError extends Schema.TaggedErrorClass<MissingMacPasskeyDomainConfigurationError>()(
  ...
  override get message(): string {
    return "CODEWORK_CLERK_PUBLISHABLE_KEY or CODEWORK_CLERK_PASSKEY_RP_DOMAINS is required for signed macOS passkey builds.";
  }
}
...
export class InvalidMacPasskeyPublishableKeyError extends Schema.TaggedErrorClass<InvalidMacPasskeyPublishableKeyError>()(
  ...
  override get message(): string {
    return "CODEWORK_CLERK_PUBLISHABLE_KEY is invalid.";
  }
}
...
export function resolveMacPasskeySigningConfiguration(
  ...
  const teamId = env.CODEWORK_APPLE_TEAM_ID?.trim().toUpperCase() ?? "";
  if (!APPLE_TEAM_ID_PATTERN.test(teamId)) {
    throw new InvalidAppleTeamIdError({ teamId });
  }

  const provisioningProfilePath = env.CODEWORK_MACOS_PROVISIONING_PROFILE?.trim() ?? "";
  if (provisioningProfilePath.length === 0) {
    throw new MissingMacPasskeyProvisioningProfileError();
  }

  const configuredRpDomains = env.CODEWORK_CLERK_PASSKEY_RP_DOMAINS?.trim();
  let rpDomains: readonly string[];
  if (configuredRpDomains) {
    rpDomains = configuredRpDomains.split(",").map(normalizePasskeyRpDomain);
  } else {
    const publishableKey = env.CODEWORK_CLERK_PUBLISHABLE_KEY?.trim();
    if (!publishableKey) {
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "CODEWORK_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("CODEWORK_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "CODEWORK_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("CODEWORK_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("CODEWORK_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("CODEWORK_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("CODEWORK_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  ...
  wslPrebuild: Config.string("CODEWORK_DESKTOP_WSL_PREBUILD").pipe(Config.option),
});
...
const verifyPackagedBundleIsSelfContained = Effect.fn("verifyPackagedBundleIsSelfContained")(
  function* (input: { readonly asarPath: string; readonly verbose: boolean }) {
    ...
    const probeRoot = yield* fs.makeTempDirectoryScoped({
      prefix: "codework-bundle-selfcheck-",
    });
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? resolveProductDisplayName("nightly")
    : (desktopPackageJson.productName ?? resolveProductDisplayName("production"));
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "Code-Work-${version}-${arch}.${ext}",
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      target: [target],
      executableName: "code-work",
      icon: "icons",
      category: "Development",
      // electron-builder turns these into MimeType=x-scheme-handler/<scheme>;
      // in the .desktop entry (Exec already gets %U), so browsers can hand
      // codework:// OAuth callbacks to the app.
      protocols: DESKTOP_PROTOCOLS,
      desktop: {
        entry: {
...
// node-pty's prebuilds/linux-<arch>/ with a codework marker the WSL preflight
...
const stageWslNodePtyPrebuild = Effect.fn("stageWslNodePtyPrebuild")(function* (input: {
  ...
  if (input.prebuildPath === undefined) {
    yield* Effect.logWarning(
      "[desktop-artifact] No WSL node-pty prebuild provided (--wsl-prebuild / CODEWORK_DESKTOP_WSL_PREBUILD); the packaged WSL backend will not start until a Linux pty.node is bundled.",
    );
    return;
  }
...
export const verifyWindowsPrimaryFffNativeLoad = Effect.fn(
  ...
  const probeRoot = yield* fs.makeTempDirectoryScoped({
    prefix: "codework-windows-primary-native-probe-",
  });
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `codework-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    codeworkCommitHash: commitHash,
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "Code Work desktop build",
    author: "Code Work Tools",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: CODEWORK_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: CODEWORK_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  ...
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: CODEWORK_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: CODEWORK_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `vp run build:desktop` and use existing dist artifacts (env: CODEWORK_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: CODEWORK_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: CODEWORK_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: CODEWORK_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: CODEWORK_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  ...
  wslPrebuild: Flag.string("wsl-prebuild").pipe(
    Flag.withDescription(
      "Path to a prebuilt Linux node-pty (pty.node) for the target arch, staged for the WSL backend (env: CODEWORK_DESKTOP_WSL_PREBUILD).",
    ),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for Code Work."),
