...
Load this reference only when inspecting or seeding local Code Work state directly.
...
```bash
node apps/server/scripts/codework-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/codework-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/codework-seed.sql
```
