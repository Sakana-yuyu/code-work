...
import { decodeRelayJwt } from "@codework/shared/relayJwt";
      ...
      return yield* Effect.tryPromise({
        try: () => readCachedClerkToken(nowMillis),
          ...
          new ManagedRelaySessionError({
            message: "Could not obtain the Code Work Connect session token.",
            cause,
          }),
      });
    }),
  ...
  return session.readClerkToken().pipe(
    Effect.flatMap((token) =>
      token
        ...
        : Effect.fail(
            new ManagedRelaySessionError({
              message: "The Code Work Connect session token is unavailable.",
            }),
          ),
    ),
  );
...
export const deregisterManagedRelayEnvironment = Effect.fn(
  ...
  if (!session || session.accountId !== input.accountId) {
    return yield* new ManagedRelaySessionError({
      message: "Sign in to Code Work Connect before deregistering an environment.",
    });
  }
...
function requireClerkToken(
  ...
  if (!session || session.accountId !== accountId) {
    return Effect.fail(
      new ManagedRelaySessionError({
        message: "Sign in to Code Work Connect before loading relay data.",
      }),
    );
  }
...
export function readManagedRelaySnapshotState<A>(
  ...
  if (result._tag === "Failure") {
    ...
    error = cause instanceof Error ? cause.message : "Could not load Code Work Connect data.";
