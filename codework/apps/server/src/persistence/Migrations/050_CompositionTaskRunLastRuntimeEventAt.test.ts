...
layer("050_CompositionTaskRunLastRuntimeEventAt", (it) => {
    ...
    Effect.gen(function* () {
      ...
      yield* sql`
        INSERT INTO composition_task_runs (
          ...
          'run-before-50', 'task-before-50', 'agent-before-50', 'runtime-before-50',
          'running', 1, '[]'
        )
      `;
      ...
      const runs = yield* sql<{
        ...
        WHERE run_id = 'run-before-50'
      `;

      assert.equal(watermark?.name, "last_runtime_event_at_unix_ms");
      assert.equal(watermark?.notnull, 0);
      assert.deepEqual(runs, [{ runId: "run-before-50", lastRuntimeEventAtUnixMs: null }]);
    }),
  );
});
