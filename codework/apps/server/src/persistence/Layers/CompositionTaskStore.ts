...
const RunLeaseCompareAndSetRequest = Schema.Struct({
  runId: Schema.String,
  taskId: Schema.String,
  agentId: Schema.String,
  runtimeId: Schema.String,
...
const makeStore = Effect.gen(function* () {
  ...
  const compareAndSetRunLeaseRow = SqlSchema.findOneOption({
    Request: RunLeaseCompareAndSetRequest,
    Result: RunRowSchema,
    execute: (input) => sql`
      UPDATE composition_task_runs
      SET lease_id = ${input.nextLeaseId}
      WHERE run_id = ${input.runId}
        ...
        AND attempt = ${input.attempt}
        AND capability_grant_ids_json = ${encodeStringArray(input.capabilityGrantIds)}
        AND lease_id IS ${input.leaseId}
        AND started_at_unix_ms IS ${input.startedAtUnixMs}
        AND last_runtime_event_at_unix_ms IS ${input.lastRuntimeEventAtUnixMs}
        AND cancel_requested_at_unix_ms IS ${input.cancelRequestedAtUnixMs}
        AND finished_at_unix_ms IS ${input.finishedAtUnixMs}
  ...
  const store: CompositionTaskStoreShape = {
    ...
    compareAndSetRunLease: (input) =>
      run(
        "CompositionTaskStore.compareAndSetRunLease",
        compareAndSetRunLeaseRow({
          runId: input.run.runId,
          taskId: input.run.taskId,
          agentId: input.run.agentId,
          runtimeId: input.run.runtimeId,
          runtimeTaskId: input.run.runtimeTaskId ?? null,
          capabilityHandshakeId: input.run.capabilityHandshakeId ?? null,
          status: input.run.status,
          attempt: input.run.attempt,
          capabilityGrantIds: [...(input.run.capabilityGrantIds ?? [])],
          leaseId: input.run.leaseId ?? null,
          startedAtUnixMs: input.run.startedAtUnixMs ?? null,
          lastRuntimeEventAtUnixMs: input.run.lastRuntimeEventAtUnixMs ?? null,
          cancelRequestedAtUnixMs: input.run.cancelRequestedAtUnixMs ?? null,
          finishedAtUnixMs: input.run.finishedAtUnixMs ?? null,
          failureCode: input.run.failureCode ?? null,
