...
const terminationInput = (input: {
  ...
  readonly exited: Deferred.Deferred<PtyAdapter.PtyExitEvent>;
  ...
  awaitExit: Deferred.await(input.exited),
...
it.effect("win32 使用无参数 kill 并等待同一 handle 的真实退出", () =>
  Effect.gen(function* () {
    ...
    const termination = yield* PtyProcessTermination.terminate(
      terminationInput({ process, platform: "win32", exited, current }),
    ...
    yield* Deferred.succeed(exited, { exitCode: 0, signal: null });
...
it.effect("Unix TERM 后自然退出会取消强杀升级", () =>
  Effect.gen(function* () {
    ...
    const finished = yield* Ref.make(false);
    const termination = yield* PtyProcessTermination.terminate(
      terminationInput({ process, platform: "linux", exited, current }),
    ).pipe(Effect.ensuring(Ref.set(finished, true)), Effect.forkChild);

    yield* Effect.yieldNow;
    expect(process.killSignals).toEqual(["SIGTERM"]);
    yield* Deferred.succeed(exited, { exitCode: 0, signal: 15 });
...
it.effect("Unix grace 到期后强杀且仍等待真实退出", () =>
  Effect.gen(function* () {
    ...
    yield* Deferred.succeed(exited, { exitCode: 137, signal: 9 });
    const outcome = yield* Fiber.join(termination);
