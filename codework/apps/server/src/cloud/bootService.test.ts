...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    unitPath: "/home/theo/.config/systemd/user/codework.service",
  });
...
const macPlan = {
  ...
  unitPath: "/Users/theo/Library/LaunchAgents/com.t3tools.codework.service.plist",
};
...
it("escapes XML in host paths", () => {
  const plist = BootService.renderBootServicePlist(
    { ...macPlan, baseDir: "/Users/theo/Code Work & <Co>" },
    { homeDir: "/Users/theo", environmentPath: "/Users/theo/Tools & <Scripts>:/usr/bin" },
  );

  expect(plist).toContain("<string>/Users/theo/Code Work &amp; &lt;Co&gt;</string>");
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now codework.service")).toEqual(
  ...
  it.effect("restarts an installed service when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("systemctl "))).toEqual([
        "systemctl --user stop codework.service",
        "systemctl --user daemon-reload",
        "systemctl --user restart codework.service",
      ]);
    }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/com.t3tools.codework.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.t3tools.codework.service",
        "launchctl enable gui/501/com.t3tools.codework.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install;
      control.failCommand = "launchctl bootout --wait gui/501/com.t3tools.codework.service";
