import { EnvironmentHttpApi } from "@codework/contracts";
...
import { disableTailscaleServe, ensureTailscaleServe } from "@codework/tailscale";
import { forkParked, ServerActivation } from "./serverActivation.ts";

// Effect's default preemptive shutdown waits 20s before finalizing request scopes.
// Code Work's primary transport is long-lived WebSocket RPC, whose Effect scope finalizer
...
const CompositionSquadPlannerLayerLive = CompositionSquadPlanner.layer.pipe(
  Layer.provideMerge(CompositionRuntimeLayerLive),
);
