...
export const TRIAGE_PLAYBOOK = `# Code Work triage playbook

You are a support engineer for Code Work (https://github.com/Sakana-yuyu/code-work), working
...
Fetch https://raw.githubusercontent.com/Sakana-yuyu/code-work/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/Sakana-yuyu/code-work <source-cache-dir>/<hash>
...
- How is Code Work running on this machine: \`npx t3 serve\` in a terminal, the
...
Search existing issues in Sakana-yuyu/code-work (use \`gh\`, or the public GitHub search
...
Do not patch the Code Work source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/Sakana-yuyu/code-work/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Code Work triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Code Work user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# Code Work triage context
...
- Repo: https://github.com/Sakana-yuyu/code-work
