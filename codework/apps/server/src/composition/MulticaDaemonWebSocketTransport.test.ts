...
const decodeSent = (socket: FakeSocketImpl): Array<{ type: string; payload?: unknown }> =>
  socket.sent.map((value) => JSON.parse(value) as { type: string; payload?: unknown });
...
describe("MulticaDaemonWebSocketTransport", () => {
  ...
  it("把匹配 request_id 的 RPC response 返回给请求方，并拒绝非 2xx", async () => {
    ...
    const request = transport.request({ method: "tasks.claim", body: { runtime_id: "runtime-1" } });
    const requestFiber = Effect.runFork(request);
    await flushEffects();
    await flushEffects();
    ...
    await expect(Effect.runPromise(Fiber.join(requestFiber))).resolves.toMatchObject({
      status: 200,
      body: { task_id: "task-1" },
    });

    const failedRequest = Effect.runFork(
    ...
    await expect(Effect.runPromise(Fiber.join(failedRequest))).rejects.toMatchObject({
      code: "rpc_error",
      status: 409,
    });
    socket.emit(
  ...
  it("RPC 在当前连接收到 rpc-v1 heartbeat ack 前等待，不会提前发送", async () => {
    const sockets: Array<FakeSocketImpl> = [];
    const transport = makeMulticaDaemonWebSocketStream({
    ...
    const requestFiber = Effect.runFork(transport.request({ method: "tasks.claim" }));
    await flushEffects();
    const socket = sockets[0]!;
    expect(socket.url).not.toContain("token=");
    socket.emit("open");
    await flushEffects();
    expect(decodeSent(socket).some((message) => message.type === "daemon:rpc_request")).toBe(false);
  ...
  it("重连后必须等待新 heartbeat ack 才能发送新的 RPC", async () => {
    const sockets: Array<FakeSocketImpl> = [];
    const transport = makeMulticaDaemonWebSocketStream({
      baseUrl: "https://multica.test",
      headers: { Authorization: "Bearer test-token" },
    ...
    const firstRequest = Effect.runFork(transport.request({ method: "tasks.claim" }));
    await flushEffects();
    ...
    await Effect.runPromise(Fiber.join(firstRequest));
    first.emit("close");
    await new Promise<void>((resolve) => setTimeout(resolve, 5));

    const second = sockets[1]!;
    second.emit("open");
    const secondRequest = Effect.runFork(transport.request({ method: "tasks.claim" }));
    await flushEffects();
    expect(decodeSent(second).some((message) => message.type === "daemon:rpc_request")).toBe(false);
    ...
    await expect(Effect.runPromise(Fiber.join(secondRequest))).resolves.toMatchObject({
      status: 200,
    });
    transport.close();
  });

  it("断线后有限重连，重连时重新发送 heartbeat，停止后清理旧连接", async () => {
    const sockets: Array<FakeSocketImpl> = [];
    const transport = makeMulticaDaemonWebSocketStream({
      baseUrl: "https://multica.test",
      headers: { Authorization: "Bearer test-token" },
