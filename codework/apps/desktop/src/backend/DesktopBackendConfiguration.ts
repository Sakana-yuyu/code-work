...
>()("@codework/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = [
  ...
  "CODEWORK_NO_BROWSER",
  "CODEWORK_HOST",
  "CODEWORK_DESKTOP_WS_URL",
  "CODEWORK_DESKTOP_LAN_ACCESS",
  "CODEWORK_DESKTOP_LAN_HOST",
  "CODEWORK_DESKTOP_HTTPS_ENDPOINTS",
  "CODEWORK_TAILSCALE_SERVE",
  "CODEWORK_TAILSCALE_SERVE_PORT",
] as const;
...
function resourceMonitorBinaryName(platform: NodeJS.Platform): string {
  return platform === "win32" ? "codework-resource-monitor.exe" : "codework-resource-monitor";
}
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    const bootstrap = {
      ...
      codeworkHome: environment.baseDir,
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's CODEWORK_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  const bootstrap = {
    ...
    noBrowser: true,
    port: input.port,
    // Omit codeworkHome so the Linux backend uses its own home dir instead of
  ...
  // Build an explicit copy of process.env minus CODEWORK_HOME (dev-runner
  ...
  const parentEnvWithoutCodeworkHome: Record<string, string | undefined> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "CODEWORK_HOME") continue;
    parentEnvWithoutCodeworkHome[key] = value;
  }
  const wslEnv = mergeWslEnv(parentEnvWithoutCodeworkHome.WSLENV, forwardedEnvNames);
  ...
  const baseConfig = {
    ...
    cwd: environment.backendCwd,
    env: {
      ...parentEnvWithoutCodeworkHome,
      ...backendChildEnvPatch(),
      ...forwardedEnv,
      ...(wslEnv !== undefined ? { WSLENV: wslEnv } : {}),
    },
    // env is already a complete process.env minus CODEWORK_HOME; pass it
