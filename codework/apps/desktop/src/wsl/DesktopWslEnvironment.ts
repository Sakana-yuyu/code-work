...
import { buildRemoteNodeEnvScript } from "@codework/ssh/tunnel";
...
>()("@codework/desktop/wsl/DesktopWslEnvironment") {}
...
const marker = path.join(prebuildDir, "codework-wsl-node-pty.json");
...
const NODE_PTY_BUILD_SCRIPT = (linuxServerDir: string) =>
  [
    ...
    `printf '{"arch":"%s","modules":"%s","nodePtyVersion":"%s"}\\n' "$arch" "$modules" "$node_pty_version" > "$prebuild_dir/codework-wsl-node-pty.json"`,
