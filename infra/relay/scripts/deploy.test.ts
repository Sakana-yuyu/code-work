...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "code-work-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "code-work-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "CODEWORK_MOBILE_OTLP_TRACES_DATASET=code-work-mobile-traces-dev",
        "CODEWORK_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "CODEWORK_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "CODEWORK_RELAY_CLIENT_OTLP_TRACES_DATASET=code-work-relay-client-traces-dev",
        "CODEWORK_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
  ...
  it("replaces stale values while preserving unrelated entries", () => {
    expect(
      reconcileRootEnvPublicConfig(
        [
          "CODEWORK_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "CODEWORK_RELAY_URL=https://old.example.test",
          "CODEWORK_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "CODEWORK_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "CODEWORK_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "CODEWORK_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "CODEWORK_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "CODEWORK_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "CODEWORK_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "CODEWORK_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "CODEWORK_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
...
describe("release workflow infrastructure boundary", () => {
  it.effect("does not require relay deployment for a desktop release", () =>
    Effect.gen(function* () {
      ...
      expect(workflow).not.toContain("needs.relay_public_config");
      expect(workflow).toContain("needs: [preflight, build_wsl_node_pty]");
