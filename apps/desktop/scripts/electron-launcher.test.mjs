...
describe("electron development launcher", () => {
  it("uses captured values only as fallbacks for a live runner environment", () => {
    ...
    assert.include(
      script,
      "exec '/repo/node_modules/electron/Electron' --codework-dev-root='/repo/apps/desktop' '/repo/apps/desktop/dist-electron/main.cjs' \"$@\"",
    );
  });
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/Code Work (Dev).app",
      "Code Work (Dev)",
    );

    assert.equal(paths.launcherExecutableName, "Code Work (Dev) Launcher");
    assert.equal(
  ...
  it("derives launcher icons from canonical development and production assets", () => {
    ...
    assert.equal(
      NodePath.normalize(development.generatedIconPath),
      NodePath.normalize("/runtime/icon-dev.icns"),
    );
    assert.match(production.sourceIconPath, /assets[\\/]prod[\\/]black-macos-1024\.png$/);
    assert.equal(
      NodePath.normalize(production.generatedIconPath),
      NodePath.normalize("/runtime/icon-prod.icns"),
    );
  });
});
