...
function startAnnotation(): void {
  ...
  cursorStyle.textContent = `html[data-codework-annotation-tool] body, html[data-codework-annotation-tool] body * { cursor: crosshair !important; } [${OVERLAY_ATTRIBUTE}], [${OVERLAY_ATTRIBUTE}] * { cursor: default !important; } [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-inner-spin-button, [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-outer-spin-button { appearance:none; margin:0; }`;
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-codework-annotation-tool", tool);
  };
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-codework-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
