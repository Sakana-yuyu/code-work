...
import { EnvironmentId, type PersistedSavedEnvironmentRecord } from "@codework/contracts";
...
function makeLayer(
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ CODEWORK_HOME: baseDir })),
    ),
  );
...
const withStore = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "codework-desktop-connection-catalog-test-",
    });
...
describe("DesktopConnectionCatalogStore", () => {
  ...
  it.effect("reports the failed catalog write operation and path", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        error.message,
        `Desktop connection catalog write failed during create-directory at ${directory}.`,
      );
      assert.notEqual(error.message, permissionError.message);
    }).pipe(Effect.provide(NodeServices.layer), Effect.scoped),
  );
