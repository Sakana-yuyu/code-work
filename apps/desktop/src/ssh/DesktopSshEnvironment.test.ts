...
import { SshPasswordPromptError } from "@codework/ssh/errors";
...
function makeTempHomeDir() {
  return Effect.gen(function* () {
    ...
    return yield* fs.makeTempDirectoryScoped({ prefix: "codework-ssh-env-test-" });
  });
}
...
describe("sshEnvironment", () => {
  it("keeps prompt presentation diagnostics distinct from the legacy wrapper message", () => {
    ...
    assert.equal(
      ...
      "Code Work window is not available for SSH authentication.",
    );
  });
