...
import { DEFAULT_CLIENT_SETTINGS } from "@codework/contracts";
...
>()("@codework/desktop/window/DesktopWindow") {}
...
export const make = Effect.gen(function* () {
  ...
  > {
    ...
    window.on("close", (event) => {
      const keepInTray =
        runSync(Ref.get(desktopState.trayReady)) && !runSync(Ref.get(desktopState.quitting));
      if (keepInTray) {
        event.preventDefault();
        window.hide();
        void runFork(logWindowInfo("main window hidden to tray"));
      }
