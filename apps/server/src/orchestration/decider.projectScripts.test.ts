...
it.layer(NodeServices.layer)("decider project scripts", (it) => {
  ...
  it.effect("emits thread.turn-start-cancelled for thread.turn.cancel", () =>
    Effect.gen(function* () {
      const now = "2026-01-01T00:00:00.000Z";
      const initial = createEmptyReadModel(now);
      const withProject = yield* projectEvent(initial, {
      ...
      const result = yield* decideOrchestrationCommand({
        command: {
          ...
          commandId: CommandId.make("cmd-turn-cancel"),
          threadId: ThreadId.make("thread-1"),
          messageId: asMessageId("queued-msg"),
          createdAt: now,
        },
        readModel,
      });
      ...
      const event = Array.isArray(result) ? result[0] : result;
      expect(event?.type).toBe("thread.turn-start-cancelled");
    }),
  );
