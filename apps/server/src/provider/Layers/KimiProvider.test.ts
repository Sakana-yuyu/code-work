...
import { KimiSettings } from "@codework/contracts";
...
const check = (input: {
  ...
  Effect.map(
    checkKimiProviderStatus(Schema.decodeSync(KimiSettings)({ enabled: input.enabled }), {}),
    (draft) => draft,
  ).pipe(
  ...
  it.effect("禁用实例也如实探测安装状态：CLI 存在时不再显示未安装", () =>
    Effect.gen(function* () {
      const draft = yield* check({
        enabled: false,
        spawner: makeStubSpawner({ code: 0, stdout: "kimi 0.41.0" }),
      });
