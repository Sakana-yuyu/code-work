...
import { deepMerge } from "@codework/shared/Struct";
...
process.env.CODEWORK_CURSOR_ENABLED = "1";
...
it.layer(
  ...
  describe("ProviderRegistryLive", () => {
    ...
    it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
      Effect.gen(function* () {
        const missingBinary = `codework_codex_missing_`;
    ...
    it.effect("re-probes when settings change the codex binaryPath", () =>
      Effect.gen(function* () {
        const firstMissing = `codework_codex_first_`;
        const secondMissing = `codework_codex_second_`;
    ...
    it.effect("skips codex probes entirely when the provider is disabled", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(status.installed, false);
        assert.strictEqual(status.message, "Codex is disabled in Code Work settings.");
      }),
    );
  });
  ...
  describe("checkClaudeProviderStatus", () => {
    ...
    it.effect("runs Claude status probes with the configured CLAUDE_CONFIG_DIR", () => {
      const claudeConfigDir = "/tmp/codework-claude-home";
