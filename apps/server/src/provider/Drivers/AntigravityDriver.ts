import { AntigravitySettings, ProviderDriverKind, type ServerProvider } from "@codework/contracts";
...
const UPDATE = makeStaticProviderMaintenanceResolver(
  makeProviderMaintenanceCapabilities({
    ...
    updateArgs: [],
    updateLockKey: null,
    // Google ships per-platform installer scripts (no npm) — the official
    // one-liners from antigravity.google/docs/cli, verbatim per platform.
    install: {
      lockKey: "antigravity-installer",
      win32: {
    ...
    Effect.gen(function* () {
      ...
      const adapter = yield* (
        makeAntigravityAdapter(effectiveConfig, {
          ...
          instanceId,
        }) as unknown as Effect.Effect<ProviderInstance["adapter"]>
      ).pipe(
      ...
      const textGeneration = yield* makeAntigravityTextGeneration(
        effectiveConfig,
        processEnv,
      ) as unknown as Effect.Effect<ProviderInstance["textGeneration"]>;
      ...
      >({
        ...
        enrichSnapshot: ({ snapshot: currentSnapshot, publishSnapshot }) =>
          enrichAntigravitySnapshot({ snapshot: stampIdentity(currentSnapshot), publishSnapshot }),
      }).pipe(
        Effect.mapError(
          (cause) =>
