 ...
 * Enable with: CODEWORK_GROK_ACP_PROBE=1 vp test run GrokAcpCliProbe
 * Set CODEWORK_GROK_LIVE_TURN=1 to also send a small prompt to the real model.
 *
...
const makeProbeRuntime = Effect.gen(function* () {
  ...
  return yield* makeGrokAcpRuntime({
    ...
    childProcessSpawner,
    cwd: process.cwd(),
    clientInfo: { name: "codework-grok-probe", version: "0.0.0" },
  });
});
...
describe.runIf(process.env.CODEWORK_GROK_ACP_PROBE === "1")("Grok ACP CLI probe", () => {
  ...
  it.effect.skipIf(process.env.CODEWORK_GROK_LIVE_TURN !== "1")(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const result = yield* runtime.prompt({
          prompt: [{ type: "text", text: "Reply exactly GROK_CODEWORK_OK. Do not use any tools." }],
        });
        yield* runtime.drainEvents;
        expect(result.stopReason).toBe("end_turn");
        expect(chunks.join("")).toContain("GROK_CODEWORK_OK");
