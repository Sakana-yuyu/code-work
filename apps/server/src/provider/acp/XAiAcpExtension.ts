...
import type { ProviderUserInputAnswers, UserInputQuestion } from "@codework/contracts";
...
/**
 * Client captured the plan for Code Work's proposed-plan card. Abandon the native
 * Grok plan-approval gate so the turn unblocks; the user implements via Code Work UI.
 */
 ...
 * Used so Code Work can show the plan while plan mode is still active (before exit).
 */
...
export const makeXAiPromptCompletionRuntime = Effect.fn("makeXAiPromptCompletionRuntime")(
  function* (runtime: AcpSessionRuntime.AcpSessionRuntime["Service"]) {
    ...
    const allocatePromptFallbackId = Effect.sync(() => {
      ...
      return `codework-xai-prompt-${nextPromptFallbackId}`;
    });
