...
import { HostProcessArguments } from "@codework/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `codework` command.
 */
...
/**
 * The `codework` package spec to suggest. The literal spec the user typed (e.g.
 * `codework@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "codework@nightly" : "codework";
}

/**
 * Render a `codework <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx codework connect` suggests
 * `npx codework serve`, a global install suggests `codework serve`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `codework ${input.subcommand}`;
  }
