...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("rejects connect commands when public configuration is missing", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["codework", "connect"]);
      assert.include(
      ...
      assert.include(output, "missing Code Work Connect public configuration");
    }).pipe(Effect.provide(Layer.mergeAll(CliRuntimeLayer, TestConsole.layer))),
  );

  it.effect("exposes service lifecycle commands without Code Work Connect configuration", () =>
    Effect.gen(function* () {
      const { output } = yield* captureStdout(runCli(["service", "--help"], noConnectCli));

      assert.include(output, "Manage the Code Work background service.");
  ...
  it.effect("exposes the Squad control command and list options", () =>
    Effect.gen(function* () {
      const { output } = yield* captureStdout(runCli(["squad", "list", "--help"], noConnectCli));

      assert.include(output, "List composition squads.");
      assert.include(output, "--server");
      assert.include(output, "--access-token");
      assert.include(output, "--include-archived");
      assert.include(output, "--json");
    }),
  );
  ...
  it.effect("reports actionable human-readable headless connect state", () =>
    Effect.gen(function* () {
      ...
      assert.include(
        output,
        "Next: Run `codework connect link` to authorize and enable Code Work Connect.",
      );
    }),
  );
  ...
  it.effect("disables headless connect without a running server", () =>
    Effect.gen(function* () {
      ...
      assert.equal(output, "Code Work Connect is disabled locally.");
    }),
  );
  ...
  it.effect("rejects invalid ttl values before running auth commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["codework", "auth", "pairing", "create"]);
  ...
  it.effect("rejects dev-url on project commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["codework", "project", "add"]);
