...
import {
  ...
  type DpopPublicJwk,
} from "@codework/shared/dpop";
import { RELAY_HEALTH_REQUEST_TYP, RELAY_MINT_REQUEST_TYP } from "@codework/shared/relayJwt";
...
import { SshLayerLive } from "./server.ts";
...
const buildAppUnderTest = (options?: {
  ...
  Effect.gen(function* () {
    ...
    const appLayer = servedRoutesLayer.pipe(
      Layer.provide(Layer.mergeAll(SshLayerLive, resourceTelemetryLayer)),
      ...
      Layer.provide(
        Layer.mock(CloudCliTokenManager.CloudCliTokenManager)({
          get: Effect.die(new Error("Unexpected Code Work Connect CLI authorization request.")),
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("reports unauthenticated session state without requiring auth", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(body.auth.sessionCookieName.startsWith("codework_session_"));
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
    ...
    Effect.gen(function* () {
      ...
      const relayConfigResponse = yield* fetchEffect(relayConfigUrl, {
        ...
        body: jsonRequestBody({
          ...
          environmentCredential: "codework_env_test_credential",
  ...
  it.effect("serves the documented Code Work Connect mint credential endpoint", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
  ...
  it.effect("serves signed Code Work Connect environment health checks", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
  ...
  for (const desktopOrigin of ["codework://app", "codework-dev://app"]) {
      ...
      Effect.gen(function* () {
        ...
        const refreshStatus = vi.fn((_: string) =>
          Effect.succeed({
            ...
            hasPrimaryRemote: true,
            isDefaultRef: false,
            refName: "codework/bootstrap-refName",
            hasWorkingTreeChanges: false,
            workingTree: {
              files: [],
              insertions: 0,
            ...
            client[ORCHESTRATION_WS_METHODS.dispatchCommand]({
              ...
              bootstrap: {
                ...
                prepareWorktree: {
                  ...
                  baseBranch: "main",
                  branch: "codework/bootstrap-refName",
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          ...
          newRefName: "codework/bootstrap-refName",
          baseRefName: "main",
          path: null,
        });
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const reportPath = yield* Config.string("CODEWORK_TRANSFER_BUDGET_REPORT_PATH").pipe(
        Config.option,
      );
      ...
      const resultPath = yield* Config.string("CODEWORK_TRANSFER_BUDGET_RESULT_PATH").pipe(
        Config.option,
      );
