...
export interface TelemetryCounters {
  ...
  readonly allCodework: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allCodework: ResourceTelemetryAggregate;
  };
...
export const emptyTelemetryCounters = (): TelemetryCounters => ({
  ...
  allCodework: emptyGroupCounters(),
});
...
function applyLifecycleCounters(input: {
  ...
  let allCodework = input.counters.allCodework;
  for (const processDelta of input.deltas) {
    ...
    allCodework = incrementCounters(allCodework, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allCodework = incrementCounters(allCodework, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allCodework = incrementCounters(allCodework, { processExits: 1 });
  }

  return { backend, electron, monitor, allCodework };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allCodework: aggregate(ordered, counters.allCodework),
    },
    deltas: processDeltas,
  };
}
