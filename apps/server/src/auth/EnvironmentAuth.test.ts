...
import { AuthAdministrativeScopes } from "@codework/contracts";
...
const makeServerConfigLayer = (overrides?: Partial<ServerConfig.ServerConfig["Service"]>) =>
  Layer.effect(
    ...
    Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "codework-auth-server-test-" })),
  );
...
it.layer(NodeServices.layer)("EnvironmentAuth.layer", (it) => {
  ...
  it.effect("keeps browsers signed in when they still present a pre-rename session cookie", () =>
    Effect.gen(function* () {
      ...
      expect(legacyCookieName).toBe(sessions.cookieName.replace(/^codework_session/, "t3_session"));
