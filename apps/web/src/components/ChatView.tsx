...
import { CHAT_LIST_ANCHOR_OFFSET } from "@codework/shared/chatList";
...
} from "@codework/shared/terminalLabels";
...
import { mergeRecentCanvasSurfaces } from "../canvas";
import { useProjectCanvasesQuery } from "./files/projectFilesQueryState";
import { useEnabledLocalPluginWorkspacePanels } from "../localPlugins/useLocalPluginWorkspacePanels";
import {
  deriveAgentPanelModel,
  foldSubagentActivities,
} from "@codework/client-runtime/state/subagentRuntime";
...
import { IdeProjectStart } from "./ide/IdeProjectStart";
import { loadWorkbenchLanguage } from "./ide/vscode/localization";
...
import {
  ...
  waitForThreadRewound,
} from "./ChatView.logic";
import type { ThreadSyncPhase } from "../threadSync";
import { useLocalStorage } from "~/hooks/useLocalStorage";
import { useComposerHandleContext } from "../composerHandleContext";
import { useEnabledLocalPluginTimelineEntries } from "../localPlugins/useLocalPluginTimelineEntries";
import {
...
const IdeWorkbench = lazy(async () => {
  await loadWorkbenchLanguage();
  return import("./ide/vscode/VscodeWorkbench");
});
...
function ChatViewContent(props: ChatViewProps) {
  ...
  const localPluginTimelineEntries = useEnabledLocalPluginTimelineEntries(routeThreadKey);
  ...
  const composerGoalObjective = useComposerDraftStore(
    (store) => store.getComposerDraft(composerDraftTarget)?.goalObjective ?? null,
  );
  ...
  const pendingCanvasOpenRef = useRef<{
    threadKey: string;
  ...
  const isIdeLayout = workspaceLayout.mode === "ide";
  ...
  const activeRightPanelKind = useRightPanelStore((state) =>
    isIdeLayout
      ? (selectSelectedRightPanelSurface(state.byThreadKey, activeThreadRef)?.kind ?? null)
  ...
  const recentCanvasSurfaces = useMemo(
    () =>
      rightPanelState.surfaces.filter(
        (surface): surface is Extract<RightPanelSurface, { kind: "canvas" }> =>
          surface.kind === "canvas" && surface.pending !== true && surface.empty !== true,
      ),
  ...
  const inlineRightPanelOwnsTitleBar = !isIdeLayout && rightPanelOpen && !shouldUseRightPanelSheet;
  ...
  const timelineMessages = useMemo(() => {
    ...
    const serverMessagesWithPresentation = serverMessagesWithPreviewHandoff.map((message) => {
      const localPresentation = localMessagePresentations[message.id];
      return localPresentation ? { ...message, localPresentation } : message;
    });
    ...
    if (localMessages.length === 0) {
      return serverMessagesWithPresentation;
    }
    const serverIds = new Set(serverMessagesWithPresentation.map((message) => message.id));
    const pendingMessages = localMessages.filter((message) => !serverIds.has(message.id));
    if (pendingMessages.length === 0) {
      return serverMessagesWithPresentation;
    }
    return [...serverMessagesWithPresentation, ...pendingMessages];
  }, [
    attachmentPreviewHandoffByMessageId,
    displayServerMessages,
    feedbackSubmissions,
  ...
  const revertTurnCountByUserMessageId = useMemo(() => {
    const byUserMessageId = new Map<MessageId, number | null>();
  ...
  // Canvases live as project files under `.codework/canvases/`, but the
  // in-memory surfaces are per-thread and die with the page. Merge the
  // project-wide on-disk listing in so the recent sidebar matches canvases
  // generated in any thread of this project, surviving reloads.
  const projectCanvases = useProjectCanvasesQuery(
    activeThreadRef?.environmentId ?? null,
    activeWorkspaceRoot,
  );
  const mergedRecentCanvasSurfaces = useMemo(
    () => mergeRecentCanvasSurfaces(recentCanvasSurfaces, projectCanvases),
    [projectCanvases, recentCanvasSurfaces],
  );
        ...
        if (!isAtomCommandInterrupted(openResult)) {
          ...
          setThreadError(
            ...
            error instanceof Error
              ? error.message
              : t("scripts.failedToRunScript", { name: script.name }),
          );
        }
        return;
      }
  ...
  const updateProjectScript = useCallback(
    async (
      ...
      if (!existingScript) {
        return AsyncResult.failure(Cause.fail(new Error(t("scripts.scriptNotFound"))));
      }
  ...
  const addDiffSurface = useCallback(() => {
    ...
    onDiffPanelOpen?.();
  }, [activeThreadRef, isGitRepo, isServerThread, onDiffPanelOpen, isIdeLayout]);
  ...
  }, [activeProject, activeThreadRef, isIdeLayout]);
  ...
  const onOpenCanvas = useCallback(
    (canvas: CanvasReference) => {
      if (!activeThreadRef || !activeProject) return;
      useRightPanelStore.getState().openCanvas(activeThreadRef, canvas);
    },
    [activeProject, activeThreadRef],
  );
  const openCanvasPanel = useCallback(() => {
    if (!activeThreadRef || !activeProject) return;
    useRightPanelStore.getState().openCanvasLauncher(activeThreadRef, t("surface.canvas"));
  }, [activeProject, activeThreadRef]);
  const onSelectCanvas = useCallback(
    (canvasId: string) => {
      if (!activeThreadRef) return;
      // Recent-list entries may exist only on disk (generated in another
      // thread or before this page load); openCanvas upserts those, and for
      // already-open canvases it just activates the existing surface.
      const canvas = mergedRecentCanvasSurfaces.find((entry) => entry.canvasId === canvasId);
      if (!canvas) return;
    ...
    [activeThreadRef, mergedRecentCanvasSurfaces],
  );
  ...
  const toggleRightPanel = useCallback(() => {
    if (isIdeLayout) {
      setIdeChatVisible((visible) => !visible);
      return;
    }
    if (!activeThreadRef) return;
    if (rightPanelOpen) {
  ...
  }, [activeThreadRef, closePreviewPanel, isIdeLayout, rightPanelOpen]);
  ...
  const closeRightPanelSurface = useCallback(
    (surface: RightPanelSurface) => {
      ...
      const activeLabel = terminalDisplayLabel(
  ...
  // the sent message can stay near the top. Code Work only owns streaming adjustments
  ...
  const handleStopBackgroundWork = useCallback(async () => {
    ...
    if (result._tag === "Failure") {
      ...
      if (!isAtomCommandInterrupted(result)) {
        ...
        setThreadError(
          activeThread.id,
          error instanceof Error ? error.message : t("backgroundWork.failedToStop"),
        );
      }
    }
  ...
  const parkedThreadBannerItem = useMemo<ComposerBannerStackItem | null>(() => {
    ...
    return {
      ...
      title: isSnoozed ? t("threadBanner.snoozed") : t("threadBanner.settled"),
  ...
  const compactDisabledReason = compactDisabled
    ? composerHasUnsentContent
      ? t("compact.reasonSendOrClearDraft")
      : !activeProject
        ? t("compact.reasonChooseProject")
        : !compactionProviderAvailable
          ? t("compact.reasonEnableClaudeProvider")
          : t("compact.reasonUnavailable")
    : null;
  ...
  const onRevertToTurnCount = useCallback(
    async (turnCount: number) => {
      ...
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        ...
        setThreadError(
          activeThread.id,
          error instanceof Error ? error.message : t("checkpoint.failedToRevert"),
        );
      }
      setIsRevertingCheckpoint(false);
    },
  ...
  const onEditUserMessage = useCallback(
    async (messageId: MessageId, text: string) => {
      ...
      if (targetTurnCount === undefined) return;
      const shouldRewind = typeof targetTurnCount === "number";
      ...
      if (shouldRewind) {
        const confirmed = await localApi.dialogs.confirm(
          [
            ...
            t("editUserMessage.rewindDescription"),
            t("checkpointRevert.warning"),
          ].join("\n"),
          { variant: "destructive" },
        );
      ...
      if (shouldRewind) {
        ...
        if (revertResult._tag === "Failure" && !isAtomCommandInterrupted(revertResult)) {
        ...
        const outcome = await waitForThreadRewound(
      ...
      const startResult = await startThreadTurn({
        ...
        input: {
          threadId: activeThread.id,
          message: {
            messageId: shouldRewind ? newMessageId() : messageId,
            role: "user",
            text: trimmed,
            attachments: [],
          },
  ...
  const onSend = async (
    ...
    const composerPreviewAnnotations =
      ...
      !sendContextPreviewAnnotations.some(
        ...
        : sendContextPreviewAnnotations;
    const promptForSend = directPrompt ?? promptRef.current;
    const {
    ...
    const hasDraftGoalObjective =
      isLocalDraftThread && (composerGoalObjective?.trim().length ?? 0) > 0;
    const goalOnlyTurn =
      hasDraftGoalObjective && (trimmed.length === 0 || trimmed === composerGoalObjective?.trim());
      ...
      toastManager.add(
        stackedThreadToast({
          ...
          actionProps: {
            children: t("copyId"),
            onClick: () => {
              void writeTextToClipboard(feedbackId, t("feedback.codexThreadIdCopyTarget")).catch(
                (error: unknown) => {
                  toastManager.add(
                    stackedThreadToast({
                      type: "error",
    ...
    if (!hasSendableContent && !hasDraftGoalObjective) {
    ...
    if (shouldCreateWorktree && !activeThreadBranch) {
      setThreadError(threadIdForSend, t("composer.selectBaseBranchForWorktree"));
      return;
    }
    ...
    const localPresentation: ChatMessagePresentation | undefined = goalOnlyTurn
      ? {
          kind: "goal",
          summary: composerGoalObjective?.trim() || trimmed,
        }
      : phase === "running"
        ? {
            kind: "queue",
            summary: messageTextForSend.trim() || trimmed,
          }
        : undefined;
    const outgoingMessageText = formatOutgoingPrompt({
      ...
      models: ctxSelectedProviderModels,
      effort: ctxSelectedPromptEffort,
      text: goalOnlyTurn ? "" : messageTextForSend || IMAGE_ONLY_BOOTSTRAP_PROMPT,
    });
    ...
    if (supportsAttachmentUploads && composerImagesSnapshot.length > 0) {
      ...
      if (getUploadedAttachments({ environmentId, images: composerImagesSnapshot }) === null) {
        ...
        setThreadError(threadIdForSend, t("composer.retryOrRemoveFailedUploads"));
        return;
      }
    }

    const resolvedSubmissionIntent = submissionIntent;
    if (
      shouldDockDraftHeroForSubmission({
        isDraftHeroState,
        activeThreadKey,
    ...
    const turnAttachmentsPromise = Promise.all(
      composerImagesSnapshot.map(async (image) => {
        if (supportsAttachmentUploads) {
          ...
          if (!uploaded) {
            throw new Error(t("composer.imageDidNotFinishUploading", { name: image.name }));
          }
          return uploaded;
        }
        return {
    ...
    if (localPresentation) {
      setLocalMessagePresentations((existing) => ({
        ...existing,
        [messageIdForSend]: localPresentation,
      }));
    }
              ...
              ...(isLocalDraftThread
                ? {
                    createThread: {
                      ...
                      ...(composerGoalObjective ? { goalObjective: composerGoalObjective } : {}),
  ...
  const addCanvasSurface = useCallback(() => {
    if (!canCreateCanvas || !activeProject || !activeWorkspaceRoot || !activeThreadKey) return;
    pendingCanvasOpenRef.current = {
      threadKey: activeThreadKey,
      threadRef: activeThreadRef,
      previousTurnId: activeLatestTurn?.turnId ?? null,
    };
    void onSend(undefined, "background", undefined, t("canvas.generatePrompt"));
  }, [
  ...
  useEffect(() => {
    const pending = pendingCanvasOpenRef.current;
    if (!pending) return;
    if (pending.threadKey !== activeThreadKey) {
      // The user moved to another thread: retire the placeholder there so no
      // orphaned spinner tab survives the generation turn.
      if (pending.threadRef) {
        useRightPanelStore.getState().closeSurface(pending.threadRef, "canvas:pending");
      }
      pendingCanvasOpenRef.current = null;
      return;
    }

    const currentTurnId = activeRunningTurnId ?? activeLatestTurn?.turnId ?? null;
    if (!currentTurnId || currentTurnId === pending.previousTurnId) return;

    // The generation turn is live: show the generating placeholder so the
    // background wait is visible instead of silent. Idempotent, and it never
    // steals the active tab back once the user switches away. openCanvas
    // retires the placeholder when the Canvas arrives; a turn that ends
    // without one closes it below.
    if (pending.threadRef) {
      useRightPanelStore
        .getState()
        .openPendingCanvas(pending.threadRef, t("canvas.generatingTitle"));
    }
    // Reverse scan without copying: this effect reruns on every projected
    // activity while a turn streams, so an O(n) array copy per event is
    // measurable churn on long-running threads.
    let canvas: (typeof workLogEntries)[number]["canvas"];
    for (let index = workLogEntries.length - 1; index >= 0; index -= 1) {
      const entry = workLogEntries[index];
      if (entry !== undefined && entry.turnId === currentTurnId && entry.canvas !== undefined) {
        canvas = entry.canvas;
        break;
      }
    }
    if (!canvas) {
      if (activeLatestTurn?.turnId === currentTurnId && activeLatestTurn.state !== "running") {
        pendingCanvasOpenRef.current = null;
        if (pending.threadRef) {
          useRightPanelStore.getState().closeSurface(pending.threadRef, "canvas:pending");
        }
  ...
  const onInterrupt = useCallback(async (): Promise<boolean> => {
    ...
    if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
      ...
      setThreadError(
        activeThread.id,
        error instanceof Error ? error.message : t("composer.failedToInterruptTurn"),
      );
      return false;
    }
    return true;
  ...
  const onRespondToApproval = useCallback(
    async (requestId: ApprovalRequestId, decision: ProviderApprovalDecision) => {
      ...
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        ...
        setThreadError(
          activeThreadId,
          error instanceof Error ? error.message : t("composer.failedToSubmitApproval"),
        );
      }
  ...
  const onRespondToUserInput = useCallback(
    async (requestId: ApprovalRequestId, answers: Record<string, unknown>) => {
      ...
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        ...
        setThreadError(
          activeThreadId,
          error instanceof Error ? error.message : t("composer.failedToSubmitUserInput"),
        );
      }
  ...
  const onSubmitPlanFollowUp = useCallback(
    async ({
      ...
      if (!isAtomCommandInterrupted(failure)) {
        ...
        setThreadError(
          threadIdForSend,
          error instanceof Error ? error.message : t("composer.failedToSendPlanFollowUp"),
        );
      }
      sendInFlightRef.current = false;
      resetLocalDispatch();
  ...
  const getModelDisabledReason = useCallback(
    (instanceId: ProviderInstanceId, model: string): string | null => {
      ...
      return reason
        ? t("modelPicker.startNewThreadToUseModel", { reason: reason.description })
        : null;
    },
  ...
  const panelToggleControls = (
    <PanelLayoutControls
      ...
      rightPanelOpen={isIdeLayout ? ideChatVisible : rightPanelOpen}
      {...(isIdeLayout ? { rightPanelLabel: t("workspace.toggleAiChat") } : {})}
      rightPanelShortcutLabel={shortcutLabelForCommand(keybindings, "rightPanel.toggle")}
      // Suppressed while the Agents surface is visible: the roster itself is
      // on screen, so the toggle badge would be pointing at nothing.
      liveAgentCount={
        isIdeLayout || (rightPanelOpen && activeRightPanelSurface?.kind === "agents")
          ? 0
          : agentPanelModel.liveCount
      }
  ...
  const panelLayoutControls = (
    ...
    >
      {canMaximizeRightPanel && !isIdeLayout ? (
  ...
  const rightPanelContent = activeThreadRef ? (
    activeRightPanelSurface?.kind === "canvas" && activeWorkspaceRoot ? (
      <CanvasPanel
        cwd={activeWorkspaceRoot}
        environmentId={activeThreadRef.environmentId}
        onOpenFile={openFileSurface}
        onCreateCanvas={addCanvasSurface}
        onSelectCanvas={onSelectCanvas}
        canCreateCanvas={canCreateCanvas}
        recentCanvases={mergedRecentCanvasSurfaces}
      ...
      <PullRequestsUnavailableState
        ...
        error={t("pullRequests.updateEnvironmentServer")}
      />
        ...
        activeProject && activeWorkspaceRoot ? (
          <Suspense fallback={null}>
            <IdeWorkbench
              ...
              cwd={activeWorkspaceRoot}
              enabled={isIdeLayout}
              relativePath={activeFileSurface?.relativePath ?? null}
              line={activeFileSurface?.revealLine ?? null}
              revealRequestId={activeFileSurface?.revealRequestId ?? 0}
              onSelection={(selection) => {
                // Editor selections update continuously while dragging; keep a
                // single live chip instead of one per intermediate event.
                const store = useComposerDraftStore.getState();
                const previous = store.getComposerDraft(composerDraftTarget)?.reviewComments ?? [];
                const others = previous.filter((entry) => !entry.sectionId.startsWith("codeoss:"));
                store.setReviewComments(composerDraftTarget, [
                  ...others,
                  {
                    id: nextFileCommentId(),
                    sectionId: `codeoss:${selection.filePath}`,
                    sectionTitle: selection.filePath,
                    filePath: selection.filePath,
                    startIndex: selection.startLine - 1,
                    endIndex: selection.endLine - 1,
                    rangeLabel: `L${selection.startLine}-L${selection.endLine}`,
                    text: t("ide.sendSelectionToChat"),
                    diff: selection.text,
                    fenceLanguage: selection.language,
                  },
                ]);
                setIdeChatVisible(true);
              }}
            />
          </Suspense>
        ) : (
          <IdeProjectStart
            ...
            loading={
              ...
              ["connected", "connecting", "reconnecting"].includes(activeEnvironmentConnectionPhase)
            }
            unavailable={activeEnvironmentUnavailable && activeReconnectingEnvironmentId === null}
            onSelectProject={(projectRef) => {
              void handleNewThread(projectRef, {
                replace: true,
                carryComposerContent: isLocalDraftThread,
              });
            }}
            onAddProject={() => openCommandPalette({ open: "add-project" })}
      ...
      header={
        ...
        >
          ...
          <ChatHeader
            ...
            keybindings={keybindings}
            availableEditors={availableEditors}
            rightPanelOpen={rightPanelOpen && !isIdeLayout}
            workspaceLayout={isIdeLayout ? "ide" : "chat"}
            onWorkspaceLayoutChange={workspaceLayout.setMode}
            gitCwd={gitCwd}
        ...
        <>
          {!shouldUseRightPanelSheet && rightPanelOpen && activeThreadRef ? (
            <RightPanelTabs
              ...
              agentsAvailable
              canvasAvailable={Boolean(activeProject && activeWorkspaceRoot)}
        ...
        >
          ...
          <div className="relative flex min-h-0 flex-1 flex-col">
            ...
            <MessagesTimeline
              ...
              localPluginTimelineEntries={localPluginTimelineEntries}
              ...
              <div className="pointer-events-auto relative z-10">
                {isDraftHeroState ? (
                  <div className="absolute inset-x-0 bottom-full z-0">
                    <div
                      className={isIdeLayout ? "pb-4" : "pb-8"}
                      style={
