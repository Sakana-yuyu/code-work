...
} from "@codework/shared/filePreview";
...
import { OfficePreviewSurface } from "./OfficePreviewSurface";
import {
...
const FILE_EXPLORER_STORAGE_KEY = "codework.fileExplorerOpen";
const RENDER_MARKDOWN_STORAGE_KEY = "codework.renderMarkdown";
...
export default function FilePreviewPanel({
  ...
  const isOffice = relativePath !== null && isWorkspaceOfficePreviewPath(relativePath);
  const file = useProjectFileQuery(environmentId, cwd, relativePath, !isImage && !isOffice);
    ...
    <div className="@container/file-preview flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-background">
        ...
        >
          ...
          {canOpenInBrowser ? (
            <Tooltip>
              <TooltipTrigger
                render={
                  <Toggle
                    ...
                    aria-label={t("openFileInPreviewBrowser")}
                    variant="ghost"
                    size="sm"
                  >
              ...
              />
              <TooltipPopup>{t("openFileInPreviewBrowser")}</TooltipPopup>
            </Tooltip>
          ) : null}
          <Tooltip>
            <TooltipTrigger
              render={
                <Toggle
                  ...
                  aria-label={explorerOpen ? t("hideFileExplorer") : t("showFileExplorer")}
                  variant="ghost"
                  size="sm"
                >
            ...
            />
            <TooltipPopup>
              {explorerOpen ? t("hideFileExplorer") : t("showFileExplorer")}
            </TooltipPopup>
          </Tooltip>
        </div>
      ) : null}
      {relativePath && file.data?.truncated ? (
        <div className="shrink-0 border-b border-warning/20 bg-warning-surface px-3 py-1.5 text-[11px] text-warning-foreground">
          {t("previewLimitedToTheFirst1MbOfA")} {file.data.byteLength.toLocaleString()}{" "}
          {t("byteFile")}
        </div>
      ) : null}
        ...
        >
          {relativePath && isImage && absolutePath ? (
            <WorkspaceImagePreview
              ...
              alt={relativePath}
            />
          ) : relativePath && isOffice && absolutePath ? (
            <OfficePreviewSurface
