...
function UnknownWhenVariableWarning({
  ...
  const label =
    identifiers.length === 1
      ? t("keybindings.unknownCondition", { name: identifiers[0] })
      : t("keybindings.unknownConditions", { names: identifiers.join(", ") });
  ...
  return (
    <Tooltip>
      ...
      <TooltipPopup side="top" className="max-w-72 whitespace-normal leading-relaxed">
        {t("keybindings.unknownConditionTooltip")}
      </TooltipPopup>
    </Tooltip>
  );
}
...
function KeybindingConflictWarning({ labels }: { labels: ReadonlyArray<string> }) {
  ...
  const conflictList =
    labels.length > 3
      ? `${labels.slice(0, 3).join(", ")}${t("keybindings.conflictsAndMore")}`
      : labels.join(", ");
  const description = t("keybindings.conflictsWith", { labels: conflictList });
  ...
  return (
    <Tooltip>
      ...
      />
      <TooltipPopup side="top" className="max-w-72 whitespace-normal leading-relaxed">
        {t("keybindings.conflictTooltip", { description })}
      </TooltipPopup>
    </Tooltip>
  );
}
      ...
      <div className="relative">
        ...
        {parseError ? (
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center rounded-lg border border-destructive/30 bg-background/75 p-4 text-center text-xs text-destructive backdrop-blur-[1px]">
            {t("keybindings.fixExpressionHint")}
          </div>
        ) : null}
      </div>
    </div>
      ...
      <div className="min-w-0 pr-4">
        <div className="flex min-w-0 items-center gap-1.5">
          <Tooltip>
            ...
            >
              {localizedCommandLabel(row.command)}
            </TooltipTrigger>
      ...
      <div className="flex min-w-0 items-center gap-2 pr-4">
        {showPill ? (
          <button
            ...
            aria-label={t("keybindings.editShortcutFor", {
          ...
          <Input
            ...
            aria-label={t("keybindings.keybindingFor", {
              command: localizedCommandLabel(row.command),
            })}
            ...
            <MenuTrigger
              render={
                <Button
                  ...
                  disabled={isSaving}
                  aria-label={t("keybindings.actionsFor", {
                    command: localizedCommandLabel(row.command),
                  })}
                />
              }
...
function NewKeybindingTableRow({
  ...
  const commandLabelText = commandDraft
    ? localizedCommandLabel(commandDraft)
    : t("keybindings.newKeybindingLabel");
  ...
  return (
    <div className="grid grid-cols-[minmax(190px,1.1fr)_minmax(220px,0.85fr)_minmax(210px,1fr)_60px] items-center px-4 py-1.5 text-sm even:bg-muted/15 hover:bg-accent/40">
          ...
          >
            {commandOptions.map((command) => (
              <SelectItem key={command} value={command} className="min-h-7 w-full py-1 text-[12px]">
                <span className="truncate">{localizedCommandLabel(command)}</span>
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      ...
      <div className="flex min-w-0 items-center gap-2 pr-4">
        <Input
          data-keybinding-capture=""
          aria-label={t("keybindings.keybindingFor", { command: commandLabelText })}
          value={isRecording ? "" : keyDraft}
          placeholder={isRecording ? t("keybindings.pressShortcut") : t("keybindings.unassigned")}
          size="compact"
...
export function KeybindingsSettingsPanel() {
  ...
  const openKeybindingsFile = useCallback(() => {
    ...
    void (async () => {
      ...
      toastManager.add({
        ...
        description:
          error instanceof Error ? error.message : t("keybindings.fileNotOpenedDescription"),
        type: "error",
      });
    })();
    ...
    (input: ServerUpsertKeybindingInput) => {
      ...
      void (async () => {
        ...
        if (!isAtomCommandInterrupted(result)) {
          const error = squashAtomCommandFailure(result);
          toastManager.add({
            title: t("unableToSaveKeybinding"),
            description:
              error instanceof Error ? error.message : t("keybindings.notSavedDescription"),
            type: "error",
          });
        }
      })();
    ...
    (row: KeybindingRow) => {
      ...
      void (async () => {
        ...
        if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
          const error = squashAtomCommandFailure(result);
          toastManager.add({
            title: t("unableToRemoveKeybinding"),
            description:
              error instanceof Error ? error.message : t("keybindings.notRemovedDescription"),
            type: "error",
          });
        }
      })();
  ...
  const bindingsCount = (
    <span className="text-[11px] text-muted-foreground">
      {t("keybindings.bindingsCount", { count: rows.length + (isAddingBinding ? 1 : 0) })}
    </span>
  );
              ...
              <TooltipTrigger
                render={
                  <Button
                    ...
                    aria-label={t("openKeybindingsJson")}
                  >
                    <FileJsonIcon className="size-3" />
                  </Button>
                }
              />
              <TooltipPopup side="top">{t("openKeybindingsJson")}</TooltipPopup>
            </Tooltip>
          </div>
        }
      >
        {!isElectron ? (
          <div className="flex items-start gap-2 border-b border-warning/20 bg-warning/5 px-3 py-2.5 text-[12px] leading-relaxed text-muted-foreground sm:px-4">
            ...
            <p>{t("keybindingsBrowserWarning")}</p>
          </div>
        ) : null}
