  ...
  it("preserves MCP server, tool, arguments, and results for expanded display", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "mcp-tool-done",
        kind: "tool.completed",
        summary: "code-work · preview_status",
        payload: {
          itemType: "mcp_tool_call",
          title: "code-work · preview_status",
          data: { item },
        },
      }),
    ];
    ...
    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.toolTitle).toBe("code-work · preview_status");
  ...
  it("keeps MCP payloads while collapsing lifecycle updates", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        ...
        kind: "tool.updated",
        summary: "code-work · preview_snapshot",
        payload: {
          itemType: "mcp_tool_call",
          toolCallId: "call-1",
          data: { item },
...
describe("formatDuration", () => {
  ...
  it("renders hours with a rounded minute remainder", () => {
    ...
    expect(formatDuration(3_600_000 * 2 + 59.4 * 60_000)).toBe("2h 59m");
