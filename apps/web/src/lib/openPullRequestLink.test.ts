...
describe("openPullRequestLink", () => {
  it("opens the requested pull request URL", async () => {
    ...
    const targetUrl = "https://github.com/Sakana-yuyu/code-work/pull/123";
  ...
  it("reports bridge failures with a safe target origin", async () => {
    ...
    const targetUrl = "https://github.com/Sakana-yuyu/code-work/pull/123?token=secret";
...
describe("parseChangeRequestUrl", () => {
  ...
  it("reads both Azure DevOps URL forms, keeping `_git` in the repository path", () => {
    expect(
      parseChangeRequestUrl("https://dev.azure.com/acme/platform/_git/codework/pullrequest/17"),
    ).toEqual({
      host: "dev.azure.com",
      repository: "acme/platform/_git/codework",
      number: 17,
    });
    expect(
      parseChangeRequestUrl("https://acme.visualstudio.com/platform/_git/codework/pullrequest/17"),
    ).toEqual({
      host: "acme.visualstudio.com",
      repository: "platform/_git/codework",
      number: 17,
    });
  });
...
describe("findProjectForChangeRequest", () => {
  ...
  it("keeps two hosts apart, so an Enterprise link does not open the public one", () => {
    const projects = [
      project({
        canonicalKey: "github.com/Sakana-yuyu/code-work",
        provider: "github",
        owner: "Sakana-yuyu",
        name: "codework",
      }),
