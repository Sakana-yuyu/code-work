...
export function NativeMarkdownSelectableText(props: {
  ...
  // CodeworkMarkdownText only rebuilds its attributed string during native layout. A
      ...
      {keyedRuns.map(({ key, run, text }) => {
        ...
        return (
          <MarkdownTextPrimitive
            ...
            nativeID={
              run.fileIcon
                ? `codework-file:${Image.resolveAssetSource(markdownFileIconSource(run.fileIcon)).uri}`
                : run.skillName
                  ? "codework-skill:sf:cube"
                  : undefined
            }
