...
public final class CodeworkKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("CodeworkKeyboardCommands")

    View(CodeworkKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: CodeworkKeyboardCommandsView, commands: [String]) in
...
public final class CodeworkKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.codeworkFirstResponder == nil else { return }
...
private extension UIView {
  var codeworkFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.codeworkFirstResponder { return responder }
    }
    return nil
  }
}
