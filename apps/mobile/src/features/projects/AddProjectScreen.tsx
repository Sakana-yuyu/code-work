...
} from "@codework/client-runtime/operations/projects";
...
} from "@codework/client-runtime/state/filesystem";
import {
  appendBrowsePathSegment,
  inferProjectTitleFromPath,
  isWindowsPlatform,
} from "@codework/client-runtime/state/projects";
import { CommandId, type EnvironmentId, ProjectId } from "@codework/contracts";
...
function SectionTitle(props: { readonly children: string }) {
  return (
    <Text className="px-1 text-2xs font-codework-bold tracking-[0.7px] uppercase text-foreground-muted">
      {props.children}
    </Text>
  );
}
    ...
    >
      <View className="flex-row items-center gap-3">
        ...
        <View className="flex-1 gap-0.5">
          <Text className="text-base leading-snug font-codework-bold">{props.title}</Text>
          {props.subtitle ? (
  ...
  return (
    ...
    >
      {props.loading ? (
        ...
        <Text className="text-base font-codework-bold text-primary-foreground">{props.label}</Text>
      )}
    </Pressable>
  );
}
...
function EmptyEnvironmentState() {
  ...
  return (
    <View className="items-center gap-3 rounded-2xl bg-card px-5 py-8">
      <Text className="text-center text-lg font-codework-bold">
        {t("commandPalette.environmentUnavailable")}
      </Text>
      <Text className="text-center text-sm leading-normal text-foreground-muted">
        {t("startOrReconnectAnEnvironmentBeforeAddingAProject")}
      </Text>
      <Pressable
...
function SourceControlRow(props: {
  ...
  const title =
    props.source === "url"
      ? "Git URL"
      : t("interface.value-repository", { value1: addProjectRemoteSourceLabel(props.source) });
  const subtitle =
    props.source === "url"
      ? t("interface.clone-from-a-remote-url")
      : t("interface.clone-value-value", {
          value1: addProjectRemoteSourceLabel(props.source),
          value2: props.hint,
        });
  const icon =
    props.source === "url" ? (
  ...
  return (
    <AddProjectShell>
      ...
      {environmentOptions.length > 1 ? (
        <>
          <SectionTitle>{t("commandPalette.environments")}</SectionTitle>
          <ListSection>
        ...
        <>
          <ListSection>
            <ListRow
              ...
              subtitle={t("commandPalette.localFolderDescription")}
              icon={
                <SymbolView
...
function useCreateProject(environment: EnvironmentOption | null) {
  ...
  return useCallback(
    async (workspaceRoot: string) => {
      ...
      if (existing) {
        Alert.alert(t("projectAlreadyExists"), existing.title);
        navigation.dispatch(
          CommonActions.reset({
            index: 0,
            routes: [
    ...
    <AddProjectShell>
      ...
      {environment ? (
        <>
          ...
          />
          <PrimaryActionButton
            label={source === "url" ? t("commandPalette.continue") : t("lookupRepository")}
    ...
    <AddProjectShell>
      ...
      {repositoryTitle ? (
        <View className="rounded-[24px] bg-card px-4 py-3">
          <Text className="text-base font-codework-bold">{repositoryTitle}</Text>
