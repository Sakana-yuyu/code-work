...
} from "@codework/shared/composerTrigger";
...
import {
  ...
  scoreQueryMatch,
} from "@codework/shared/searchRanking";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text
          className="max-w-[260px] text-sm font-codework-bold leading-snug text-foreground"
          numberOfLines={1}
        >
          {props.status.label}
        </Text>
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      armAgentAwarenessLiveActivityForLocalWork({
        ...
        projectTitle: props.environmentLabel ?? "Code Work",
      });
    } finally {
            ...
            <View className="flex-row gap-1 pl-1">
              ...
              {props.draftAttachments.length > 3 ? (
                <View className="size-[30px] items-center justify-center rounded-lg bg-subtle-strong">
                  <Text className="text-foreground-muted text-2xs font-codework-bold">
                    +{props.draftAttachments.length - 3}
                  </Text>
                </View>
              ) : null}
