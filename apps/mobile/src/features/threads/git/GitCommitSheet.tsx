...
export function GitCommitSheet(_props: GitCommitSheetProps) {
  ...
  return (
    <View collapsable={false} className="flex-1 bg-sheet">
      {Platform.OS === "android" ? (
        <AndroidSheetHeader title={t("gitDialog.commitTitle")} onBack={() => navigation.goBack()} />
      ) : null}
      <ScrollView
        className="flex-1"
      ...
      >
        <View className="gap-3 rounded-[22px] border border-border bg-card px-4 py-4">
          <View className="flex-row items-center justify-between gap-3">
            ...
            <Text className="text-foreground text-base font-codework-bold">
              {gitStatus.data?.refName ?? t("detachedHead")}
            </Text>
          </View>
          {isDefaultRef ? (
            <Text className="text-xs leading-normal text-amber-700 dark:text-amber-400">
              {t("warningThisIsTheDefaultBranch")}
            </Text>
          ) : null}
        </View>
        ...
        <View className="gap-3 rounded-[22px] border border-border bg-card px-4 py-4">
          <View className="flex-row items-center justify-between gap-3">
            ...
            <View className="flex-row items-center gap-2">
              {!allSelected && isEditingFiles ? (
                ...
                >
                  <Text className="text-foreground text-2xs font-codework-bold uppercase">
                    {t("reset")}
                  </Text>
                </Pressable>
              ) : null}
          ...
          {allFiles.length === 0 ? (
            <Text className="text-foreground-secondary text-sm leading-normal">
              {t("noChangedFilesAreAvailableToCommit")}
            </Text>
          ) : !isEditingFiles ? (
            <View className="gap-2">
              {selectedFilePreview.map((file) => (
                <View key={file.path} className="flex-row items-center justify-between gap-3">
                  ...
                  <Text className="text-xs font-codework-bold text-emerald-500">
                    +{file.insertions}
                  </Text>
                  <Text className="text-xs font-codework-bold text-rose-500">
                    -{file.deletions}
                  </Text>
                </View>
              ))}
              {selectedFiles.length > selectedFilePreview.length ? (
                <Text className="text-foreground-muted text-xs leading-snug">
                  +{selectedFiles.length - selectedFilePreview.length} {t("moreFiles")}
                </Text>
              ) : null}
            </View>
          ) : (
                  ...
                  >
                    ...
                    <View className="flex-row items-start justify-between gap-3">
                      <View className="flex-1 gap-1">
                        <Text
                          selectable
                          className={`text-sm font-codework-bold ${included ? "text-foreground" : "text-foreground-muted"}`}
                        >
                          {file.path}
                        </Text>
                        {!included ? (
                          <Text className="text-foreground-muted text-2xs leading-normal">
                            {t("excludedFromThisCommit")}
                          </Text>
                        ) : null}
                      </View>
        ...
        <View className="gap-2">
          <Text className="text-foreground text-sm font-codework-bold">{t("commitMessage")}</Text>
          <TextInput
            multiline
