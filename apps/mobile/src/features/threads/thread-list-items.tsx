    ...
    >
      ...
      >
        ...
        <Text
          className={
            compact
              ? "flex-shrink text-base font-codework-bold tracking-[0.2px] text-foreground-muted"
              : "flex-shrink text-sm font-codework-bold tracking-[0.2px] text-foreground-muted"
          }
          numberOfLines={1}
        >
          {props.title}
        ...
        <Text
          className={
            compact
              ? "flex-1 text-sm font-codework-medium text-foreground-tertiary"
              : "flex-1 text-xs font-codework-medium text-foreground-tertiary"
          }
        >
          {props.threadCount}
        </Text>
    ...
    >
      <View className="flex-row items-center gap-1.5">
        ...
        <Text
          className={
            compact
              ? "text-sm font-codework-medium text-foreground-muted"
...
export const PendingTaskListRow = memo(function PendingTaskListRow(props: {
  ...
  const statusPill = (
    <View className="rounded-full bg-zinc-500/12 px-1.5 py-0.5 dark:bg-zinc-500/16">
      <Text className="text-3xs font-codework-bold text-zinc-600 dark:text-zinc-300">
        {t("pending")}
      </Text>
    </View>
  );
  ...
  const rowContent = compact ? (
      ...
      >
        ...
        >
          <View className="flex-row items-center justify-between gap-2">
            <Text className="flex-1 text-lg font-codework-bold text-foreground" numberOfLines={1}>
              {pendingTask.title}
            </Text>
    ...
    >
      <View className="gap-[3px]">
        <View className="flex-row items-center justify-between gap-2">
          <Text className="flex-1 text-base font-codework-medium text-foreground" numberOfLines={1}>
            {pendingTask.title}
          </Text>
          <View className="flex-row items-center gap-2">
            {statusPill}
...
export const ThreadListRow = memo(function ThreadListRow(props: {
  ...
  const statusPill = effectiveStatus ? (
    <View className={`${effectiveStatus.pillClassName} rounded-full px-1.5 py-0.5`}>
      <Text className={`text-3xs font-codework-bold ${effectiveStatus.textClassName}`}>
      ...
      <View className="mt-px flex-row items-center gap-1.5">
        ...
        {pr !== null ? (
          <View className="flex-row items-center gap-0.5">
            ...
            />
            <Text
              className={`${compact ? "text-sm" : "text-xs"} font-codework-medium ${
      ...
      >
        <View className="gap-[3px]">
          <View className="flex-row items-center justify-between gap-2">
            <Text
              className={cn(
                "flex-1 text-base font-codework-medium",
