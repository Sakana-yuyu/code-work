...
describe("tryOpenExternalUrl", () => {
  it("opens supported URLs", async () => {
    ...
    await expect(
      tryOpenExternalUrl("https://github.com/Sakana-yuyu/code-work", "pull-request"),
    ).resolves.toBe(true);
  });
  ...
  it("logs stable URL context without exposing the opening failure", async () => {
    ...
    await expect(
      tryOpenExternalUrl("https://github.com/Sakana-yuyu/code-work/pull/1?token=secret", "pull-request"),
    ).resolves.toBe(false);
