...
const CALLS_KEY = "code-work.browser-preview.calls";
...
test("刷新探测、启用开关和优先级都走真实保存入口", async ({ page }) => {
  ...
  await priority.blur();
  await expect.poll(() => page.evaluate((key) => JSON.parse(localStorage.getItem(key) || "[]").map((item) => item.name), CALLS_KEY)).toContain("RefreshDelegationExecutorProbes");
  await expect.poll(() => page.evaluate(() => JSON.parse(localStorage.getItem("code-work.browser-preview.config") || "{}")?.delegation?.executors?.find((item) => item.id === "codex-cli")?.priority)).toBe(5);
  await expect.poll(() => page.evaluate(() => JSON.parse(localStorage.getItem("code-work.browser-preview.config") || "{}")?.delegation?.executors?.find((item) => item.id === "gemini-cli")?.enabled)).toBe(true);
});
