...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem("code-work.browser-preview.test-plan", JSON.stringify({}));
    localStorage.removeItem("code-work.browser-preview.config");
    if (!sessionStorage.getItem("code-work.history.e2e-initialized")) {
      localStorage.removeItem("code-work.history.view-mode");
      sessionStorage.setItem("code-work.history.e2e-initialized", "true");
    }
  });
});
...
test("图标和详细信息视图共享选择并记住最后使用的模式", async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => localStorage.getItem("code-work.history.view-mode"))).toBe("details");
