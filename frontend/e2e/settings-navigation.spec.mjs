...
test("更多设置默认展开且所有分类使用一致的选中状态", async ({ page }) => {
  await page.addInitScript(() => {
    ...
    localStorage.removeItem("code-work.browser-preview.config");
    localStorage.removeItem("code-work.settings.category");
  });
...
test("分类选择在侧栏、内容、URL、存储和浏览器历史间保持同步", async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => localStorage.getItem("code-work.settings.category"))).toBe("prompts");
...
test("设置页返回始终进入服务控制台，不受进入设置前的历史页面影响", async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem(
      "code-work.browser-preview.test-plan",
      JSON.stringify({}),
