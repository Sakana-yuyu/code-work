...
test("浏览器预览用内存工作区浏览、读取和搜索，不暴露主机路径", async ({ page }) => {
  ...
  await expect(page.getByLabel("源代码", { exact: true })).toHaveText("分支 main · 领先 1 · 落后 2");
...
test("工作区主列是编辑器而不是运维表单墙", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Git 操作" })).toHaveCount(0);
  await expect(page.getByRole("heading", { name: "已知主机" })).toHaveCount(0);
  await expect(page.getByRole("heading", { name: "终端" })).toHaveCount(0);
  await expect(page.getByLabel("AI 对话")).toBeVisible();
  const body = await page.locator("body").innerText();
  assertNoHostLeak(body);
});

test.describe("设置中的 Git SSH", () => {
  test.skip("SSH 密钥导入与已知主机审批", async ({ page }) => {
    await openIde(page);
